...
<div class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-sm px-4" x-show="partModal" x-cloak @keydown.escape.window="partModal = false">
    <div class="w-full max-w-3xl bg-white rounded-2xl shadow-xl border border-slate-200 max-h-[92vh] overflow-y-auto">
        <div class="flex items-center justify-between px-5 py-4 border-b border-slate-200 sticky top-0 bg-white z-10">
            <div class="text-sm font-semibold text-slate-900" x-text="partMode === 'create' ? '{{ __('master.parts.part_modal.add_title') }}' : '{{ __('master.parts.part_modal.edit_title') }}'"></div>
        ...
        <form :action="partAction" method="POST" class="px-5 py-4 space-y-5" x-ref="partForm">
            ...
            <section>
                <h3 class="text-[11px] font-semibold uppercase tracking-wider text-slate-400 mb-2">{{ __('master.parts.part_modal.section_identity') }}</h3>
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div>
                        <label for="part_no" class="text-sm font-semibold text-slate-700">{{ __('master.parts.part_modal.part_no') }} <span class="text-red-600">*</span></label>
                        <input id="part_no" name="part_no" required class="mt-1 w-full rounded-lg border-slate-300" x-model="partForm.part_no">
                    </div>
                    <div>
                        <label for="classification" class="text-sm font-semibold text-slate-700">{{ __('master.parts.part_modal.type') }} <span class="text-red-600">*</span></label>
                ...
                <div class="mt-3">
                    <label for="part_name" class="text-sm font-semibold text-slate-700">{{ __('master.parts.part_modal.part_name') }}</label>
                    <input id="part_name" name="part_name" class="mt-1 w-full rounded-lg border-slate-300" x-model="partForm.part_name">
                </div>
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-3">
                    <div>
                        <label for="size" class="text-sm font-semibold text-slate-700">{{ __('master.parts.part_modal.size') }}</label>
                        <input id="size" name="size" class="mt-1 w-full rounded-lg border-slate-300" placeholder="{{ __('master.parts.part_modal.size_placeholder') }}" x-model="partForm.size">
                    </div>
                    <div x-show="partForm.classification !== 'RM'" x-cloak>
                        <label for="model" class="text-sm font-semibold text-slate-700">{{ __('master.parts.part_modal.model') }}</label>
                        <input id="model" name="model" class="mt-1 w-full rounded-lg border-slate-300" x-model="partForm.model">
                    </div>
                </div>
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-3">
                    <div>
                        <label for="uom" class="text-sm font-semibold text-slate-700">{{ __('master.parts.part_modal.uom_stock') }}</label>
                        <select id="uom" name="uom" class="mt-1 w-full rounded-lg border-slate-300" x-model="partForm.uom">
                        ...
                        <p class="mt-1 text-xs text-slate-400">{{ __('master.parts.part_modal.uom_stock_hint') }}</p>
                    </div>
                    <div></div>
                </div>
            </section>
            ...
            <section>
                <h3 class="text-[11px] font-semibold uppercase tracking-wider text-slate-400 mb-2">{{ __('master.parts.part_modal.section_material') }}</h3>
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div>
                        <label for="consumption_policy" class="text-sm font-semibold text-slate-700">{{ __('master.parts.part_modal.material_policy') }}</label>
                        <select id="consumption_policy" name="consumption_policy" class="mt-1 w-full rounded-lg border-slate-300" x-model="partForm.consumption_policy">
                            <option value="direct_issue">{{ __('master.parts.part_modal.policy_direct') }}</option>
                            <option value="backflush_return">{{ __('master.parts.part_modal.policy_return') }}</option>
                            <option value="backflush_line_stock">{{ __('master.parts.part_modal.policy_line') }}</option>
                        </select>
                    </div>
                    <div>
                        <label for="status" class="text-sm font-semibold text-slate-700">{{ __('master.parts.part_modal.status') }}</label>
                        <select id="status" name="status" class="mt-1 w-full rounded-lg border-slate-300" x-model="partForm.status">
                            <option value="active">{{ __('master.parts.part_modal.status_active') }}</option>
                            <option value="inactive">{{ __('master.parts.part_modal.status_inactive') }}</option>
                        </select>
                    </div>
                </div>
            </section>
            ...
            <template x-if="partForm.classification === 'RM'">
                <section class="rounded-xl border border-slate-200 overflow-hidden">
                    ...
                    <div class="flex items-center justify-between px-3.5 py-2.5 bg-slate-50 border-b border-slate-200">
                        <div>
                            <span class="text-sm font-semibold text-slate-700">{{ __('master.parts.part_modal.vendor_material_title') }}</span>
                        ...
                        <button type="button" @click="addVendorRow()" class="px-3 py-1.5 rounded-lg bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-semibold">{{ __('master.parts.part_modal.add_vendor') }}</button>
                    </div>
                        ...
                        <template x-for="(vp, idx) in partForm.vendor_parts" :key="vp._key">
                            <div class="rounded-lg border border-slate-200 p-3 space-y-2.5 bg-white">
                                <div class="flex items-center justify-between">
                                    <span class="text-[10px] font-bold uppercase tracking-wider text-slate-400" x-text="'{{ __('master.parts.part_modal.vendor_n') }} ' + (idx + 1)"></span>
                                    <button type="button" @click="removeVendorRow(idx)" class="text-xs font-semibold text-red-600 hover:text-red-800">{{ __('master.parts.part_modal.delete') }}</button>
                                </div>
                                <input type="hidden" :name="'vendor_parts[' + idx + '][id]'" :value="vp.id">
                                <div class="grid grid-cols-1 sm:grid-cols-3 gap-2.5">
                                    <div class="col-span-1 sm:col-span-2">
                                        <label class="text-xs font-semibold text-slate-600">{{ __('master.parts.part_modal.vendor_label') }} <span class="text-red-600">*</span></label>
                                        <select :name="'vendor_parts[' + idx + '][vendor_id]'" required class="mt-0.5 w-full rounded-lg border-slate-300 text-sm" x-model="vp.vendor_id" @change="ensureVpNames(vp.vendor_id)">
                                            <option value="">{{ __('master.parts.part_modal.vendor_select') }}</option>
                                    ...
                                    <div>
                                        <label class="text-xs font-semibold text-slate-600">{{ __('master.parts.part_modal.vendor_status') }}</label>
                                ...
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                                    <div>
                                        <label class="text-xs font-semibold text-slate-600">{{ __('master.parts.part_modal.vendor_part_no') }}</label>
                                    ...
                                    <div>
                                        <label class="text-xs font-semibold text-slate-600">{{ __('master.parts.part_modal.register_no') }}</label>
                                ...
                                <div>
                                    <label class="text-xs font-semibold text-slate-600">{{ __('master.parts.part_modal.material_group') }} <span class="font-normal text-slate-400">{{ __('master.parts.part_modal.material_group_suffix') }}</span></label>
                                    <input type="text" :name="'vendor_parts[' + idx + '][vendor_part_name]'" :list="'vp-names-' + vp._key"
                                        class="mt-0.5 w-full rounded-lg border-slate-300 text-sm" placeholder="{{ __('master.parts.part_modal.material_group_placeholder') }}"
                                ...
                                <div class="grid grid-cols-1 sm:grid-cols-3 gap-2.5 items-end">
                                    <div>
                                        <label class="text-xs font-semibold text-slate-600">{{ __('master.parts.part_modal.uom') }}</label>
                                    ...
                                    <div>
                                        <label class="text-xs font-semibold text-slate-600">{{ __('master.parts.part_modal.hs_code') }}</label>
                                    ...
                                    <label class="flex items-center gap-2 text-sm text-slate-700 pb-1.5">
                                        ...
                                        <span class="text-xs font-semibold">{{ __('master.parts.part_modal.qc') }}</span>
                                    </label>
                                </div>
                            </div>
                        </template>
                        <p x-show="!partForm.vendor_parts.length" class="text-xs text-slate-400 text-center py-3">
                            {{ __('master.parts.part_modal.empty_vendor') }}
                        </p>
                        <p class="text-[11px] text-slate-400">{{ __('master.parts.part_modal.pricing_note_before') }} <a href="{{ route('pricing.index') }}" class="font-semibold text-indigo-600 hover:underline">{{ __('master.parts.part_modal.pricing_note_link') }}</a>{{ __('master.parts.part_modal.pricing_note_after') }}</p>
                    </div>
                </section>
            </template>
            ...
            <template x-if="partForm.classification === 'FG' || partForm.classification === 'WIP'">
                <section x-data="{ open: false }" class="rounded-xl border border-slate-200 overflow-hidden">
                    <button type="button" @click="open = !open" class="w-full flex items-center justify-between px-3.5 py-2.5 text-sm font-semibold text-slate-700 hover:bg-slate-50">
                        <span>{{ __('master.parts.part_modal.customer_title') }}</span>
                    ...
                    <div x-show="open" x-cloak class="px-3.5 pb-3">
                        ...
                        <p class="mt-2 text-[11px] text-slate-400">{{ __('master.parts.part_modal.customer_hint') }}</p>
                    </div>
                </section>
            </template>
            ...
            <template x-if="partForm.classification === 'FG' || partForm.classification === 'WIP'">
                <section x-data="{ open: false }" class="rounded-xl border border-slate-200 overflow-hidden">
                    <button type="button" @click="open = !open" class="w-full flex items-center justify-between px-3.5 py-2.5 text-sm font-semibold text-slate-700 hover:bg-slate-50">
                        <span>{{ __('master.parts.part_modal.subcount_title') }}</span>
                        <span class="inline-flex items-center gap-2">
                            <span class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold" :class="partForm.subcount_enabled ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-400'" x-text="partForm.subcount_enabled ? '{{ __('master.parts.part_modal.subcount_active') }}' : '{{ __('master.parts.part_modal.subcount_inactive') }}'"></span>
                            <svg :class="open && 'rotate-180'" class="h-4 w-4 text-slate-400 transition-transform" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="m6 9 6 6 6-6"/></svg>
                        </span>
                    </button>
                    <div x-show="open" x-cloak class="px-3.5 pb-3 space-y-3">
                        <label class="flex items-center justify-between text-sm">
                            <span class="font-semibold text-slate-700">{{ __('master.parts.part_modal.subcount_enable') }}</span>
                            <span class="inline-flex items-center gap-2 text-slate-600">
                                ...
                                {{ __('master.parts.part_modal.subcount_active_label') }}
                            </span>
                        </label>
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3" x-show="partForm.subcount_enabled" x-cloak>
                            <div>
                                <label class="text-xs font-semibold text-slate-600">{{ __('master.parts.part_modal.subcount_fg') }}</label>
                                <select name="subcount_fg_part_id" class="mt-1 w-full rounded-lg border-slate-300 text-sm" x-model="partForm.subcount_fg_part_id">
                                    <option value="">{{ __('master.parts.part_modal.subcount_fg_self') }}</option>
                            ...
                            <div>
                                <label for="subcount_rm_part_id" class="text-xs font-semibold text-slate-600">{{ __('master.parts.part_modal.subcount_rm') }}</label>
                                <select id="subcount_rm_part_id" name="subcount_rm_part_id" class="mt-1 w-full rounded-lg border-slate-300 text-sm" x-model="partForm.subcount_rm_part_id">
                                    <option value="">{{ __('master.parts.part_modal.subcount_rm_self') }}</option>
                            ...
                            <div>
                                <label for="subcount_uom" class="text-xs font-semibold text-slate-600">{{ __('master.parts.part_modal.subcount_uom') }}</label>
                            ...
                            <div>
                                <label for="subcount_process_type" class="text-xs font-semibold text-slate-600">{{ __('master.parts.part_modal.subcount_process') }}</label>
            ...
            <div class="flex justify-end gap-2 pt-1 border-t border-slate-100">
                <button type="button" class="px-4 py-2 rounded-lg border border-slate-200 hover:bg-slate-50 text-sm font-medium" @click="partModal = false">{{ __('master.parts.part_modal.cancel') }}</button>
                <button type="submit" class="px-4 py-2 rounded-lg bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-semibold">{{ __('master.parts.part_modal.save') }}</button>
            </div>
        </form>
    </div>
</div>
