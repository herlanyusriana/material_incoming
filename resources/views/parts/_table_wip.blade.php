        ...
        <table class="min-w-full text-sm divide-y divide-slate-200">
            <thead class="bg-slate-50 text-slate-600 text-xs font-semibold uppercase tracking-wider">
                <tr>
                    ...
                    <th class="px-3 py-3 text-left font-semibold">{{ __('master.parts.table_wip.th_part') }}</th>
                    <th class="px-3 py-3 text-left font-semibold">{{ __('master.parts.table_wip.th_model') }}</th>
                    <th class="px-3 py-3 text-left font-semibold">{{ __('master.parts.table_wip.th_policy') }}</th>
                    <th class="px-3 py-3 text-left font-semibold">{{ __('master.parts.table_wip.th_status') }}</th>
                ...
                @forelse ($parts as $p)
                    <tr class="hover:bg-slate-50">
                        ...
                        <td class="px-3 py-2.5 text-right whitespace-nowrap">
                            <button type="button" class="font-semibold text-indigo-600 hover:text-indigo-800 text-sm" @click="openEditPart(@js($p))">{{ __('master.parts.table_wip.edit') }}</button>
                            <form action="{{ route('parts.destroy', $p) }}" method="POST" class="inline ml-2" onsubmit="return confirm(@js(__('master.parts.table_wip.delete_confirm', ['no' => $p->part_no])))">
                                @csrf @method('DELETE')
                                <button type="submit" class="font-semibold text-red-600 hover:text-red-800 text-sm">{{ __('master.parts.table_wip.delete') }}</button>
                            </form>
                        </td>
                    </tr>
                @empty
                    <tr><td colspan="6" class="px-4 py-16 text-center">
                        ...
                        <p class="mt-3 text-sm font-semibold text-slate-600">{{ __('master.parts.table_wip.empty_title') }}</p>
                        <p class="mt-1 text-xs text-slate-400">{{ __('master.parts.table_wip.empty_hint') }}</p>
                        <button type="button" @click="openCreatePart()" class="mt-4 inline-flex items-center gap-1.5 px-3.5 py-2 rounded-lg bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-semibold">{{ __('master.parts.table_wip.add_part') }}</button>
                    </td></tr>
                @endforelse
            </tbody>
        </table>
