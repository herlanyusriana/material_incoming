<x-app-layout>
            ...
            <form action="{{ route('local-pos.update', $arrival) }}" method="POST" enctype="multipart/form-data"
                ...
                <div class="grid md:grid-cols-4 gap-4">
                    <div class="md:col-span-2">
                        ...
                        <select name="vendor_id" class="mt-1 w-full rounded-xl border-slate-200" required id="vendor-select">
                            ...
                            @foreach ($vendors as $v)
                                <option value="{{ $v->id }}" @selected((string) old('vendor_id', $arrival->vendor_id) === (string) $v->id)>{{ $v->vendor_name }}</option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <input type="date" name="po_date" value="{{ old('po_date', $arrival->invoice_date?->format('Y-m-d')) }}"
                    ...
                    <div class="overflow-x-auto border border-slate-200 rounded-xl">
                        <table class="min-w-full divide-y divide-slate-200 text-sm" id="items-table">
                            ...
                            <tbody class="divide-y divide-slate-100" id="items-tbody">
                                @foreach (old('items', $arrival->items) as $i => $item)
                                    @php
                                        // Handle both Model and Array (from old input)
                                        $itemId = is_array($item) ? ($item['id'] ?? null) : $item->id;
                                        $partId = is_array($item) ? ($item['part_id'] ?? '') : $item->part_id;
                                        $size = is_array($item) ? ($item['size'] ?? '') : $item->size;
                                        $qty = is_array($item) ? ($item['qty_goods'] ?? 0) : $item->qty_goods;
                                        $unit = is_array($item) ? ($item['unit_goods'] ?? 'PCS') : $item->unit_goods;
                                        $price = is_array($item) ? ($item['price'] ?? 0) : $item->price;
                                        
                                        // Check if has receives? Not easily available in old array, but available in model.
                                        // But we passed data via controller.
                                        // For simplicity, we won't strictly disable "inputs" here based on receives in blade unless critical.
                                        // Backend validation is key.
                                    @endphp
                                    <tr class="hover:bg-slate-50">
                                        <td class="px-4 py-3">
                                            <input type="hidden" name="items[{{ $i }}][id]" value="{{ $itemId }}">
                                            <select name="items[{{ $i }}][part_id]" class="w-72 rounded-xl border-slate-200" required data-part-select data-old="{{ $partId }}">
                                                {{-- Options populated by JS --}}
                                            </select>
                                        </td>
                                        <td class="px-4 py-3">
                                            <input type="text" name="items[{{ $i }}][size]" class="w-40 rounded-xl border-slate-200 uppercase"
                                                placeholder="0.7 X 530 X C" value="{{ $size }}">
                                        </td>
                                        <td class="px-4 py-3">
                                            <div class="flex items-center justify-end gap-2">
                                                <input type="number" name="items[{{ $i }}][qty_goods]" min="0" step="1"
                                                    value="{{ $qty }}" class="w-24 text-right rounded-xl border-slate-200" required>
                                                <select name="items[{{ $i }}][unit_goods]" class="w-24 rounded-xl border-slate-200" required>
                                                    <option value="PCS" @selected($unit === 'PCS')>PCS</option>
                                                    <option value="COIL" @selected($unit === 'COIL')>COIL</option>
                                                    <option value="SHEET" @selected($unit === 'SHEET')>SHEET</option>
                                                    <option value="SET" @selected($unit === 'SET')>SET</option>
                                                    <option value="EA" @selected($unit === 'EA')>EA</option>
                                                    <option value="ROLL" @selected($unit === 'ROLL')>ROLL</option>
                                                    <option value="KGM" @selected($unit === 'KGM')>KGM</option>
                                                </select>
                                            </div>
                                        </td>
                                        <td class="px-4 py-3">
                                            <input type="number" name="items[{{ $i }}][price]" step="0.01" min="0"
                                                value="{{ $price }}" class="w-32 text-right rounded-xl border-slate-200" placeholder="0">
                                        </td>
    ...
    <script>
        (function () {
            ...
            // Adjust idx based on existing count
            let idx = {{ count(old('items', $arrival->items)) + 1 }};
            ...
            function setPartsOptions(selectEl) {
                ...
                if (desired) {
                    ...
                    if (exists) {
                        ...
                        // Keep dataset old?? maybe useless after first set
                    }
                }
            }
            ...
            vendorSelect?.addEventListener('change', () => {
                // Warning: changing vendor clears parts logic normally, but let's just re-render.
                // If items exist for other vendor, they will be invalid.
            ...
            // Auto-fill price, size/storage_reg when part is selected
            ...
            // Initialize all rows with correct parts options
