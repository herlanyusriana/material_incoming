<x-app-layout>
    <x-slot name="header">{{ __('subcon.subcounts.show.header') }}</x-slot>
        ...
        <div class="mx-auto max-w-7xl space-y-6 px-4 sm:px-6 lg:px-8">
            <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                <div class="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                    <div>
                        <a href="{{ route('subcounts.index') }}" class="text-xs font-bold uppercase tracking-wider text-slate-400 hover:text-slate-700">&larr; {{ __('subcon.subcounts.show.back') }}</a>
                    ...
                    <div class="rounded-xl bg-blue-50 px-4 py-3 text-right">
                        <div class="text-xs font-bold uppercase tracking-wider text-blue-700">{{ __('subcon.subcounts.show.total_net') }}</div>
            ...
            <div class="grid gap-4 md:grid-cols-4">
                <div class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                    <div class="text-xs font-bold uppercase tracking-wider text-slate-500">{{ __('subcon.subcounts.show.wh_send') }}</div>
                ...
                <div class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                    <div class="text-xs font-bold uppercase tracking-wider text-slate-500">{{ __('subcon.subcounts.show.part_job_lot') }}</div>
                    <div class="mt-1 font-bold text-slate-900">{{ $subcount->part_info ?? '-' }}</div>
                </div>
                <div class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                    <div class="text-xs font-bold uppercase tracking-wider text-slate-500">{{ __('subcon.subcounts.show.operator') }}</div>
                    <div class="mt-1 font-bold text-slate-900">{{ $subcount->operator_name ?? '-' }}</div>
                </div>
                <div class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                    <div class="text-xs font-bold uppercase tracking-wider text-slate-500">{{ __('subcon.subcounts.show.records') }}</div>
                    <div class="mt-1 font-bold text-slate-900">{{ number_format($subcount->records->count()) }}</div>
                </div>
                <div class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                    <div class="text-xs font-bold uppercase tracking-wider text-slate-500">{{ __('subcon.subcounts.show.received') }}</div>
                ...
                @forelse ($subcount->records as $record)
                    <div class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                        <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                            <div>
                                <div class="text-xs font-bold uppercase tracking-wider text-slate-500">{{ __('subcon.subcounts.show.packaging') }}</div>
                            ...
                            <div class="grid grid-cols-2 gap-2 text-center text-xs">
                                <div class="rounded-xl bg-slate-50 px-3 py-2">
                                    <div class="font-bold text-slate-500">{{ __('subcon.subcounts.show.gross') }}</div>
                                ...
                                <div @class([
                                    ...
                                    ])>{{ __('subcon.subcounts.show.net') }}</div>
                        ...
                        <div class="mt-4 grid gap-4 lg:grid-cols-[minmax(0,1fr)_360px]">
                            <figure class="overflow-hidden rounded-2xl border border-slate-200 bg-slate-50">
                                @if ($record->gross_photo_path)
                                    <img src="{{ Storage::disk('public')->url($record->gross_photo_path) }}" alt="{{ __('subcon.subcounts.index.photo_alt') }}" class="aspect-[4/3] w-full object-cover">
                                @endif
                                <figcaption class="px-4 py-3 text-xs font-bold uppercase tracking-wider text-slate-500">{{ __('subcon.subcounts.show.realtime_photo') }}</figcaption>
                            </figure>
                            <form method="POST" action="{{ route('subcounts.records.netto', $record) }}" class="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                                @csrf
                                @method('PUT')
                                <div class="text-sm font-black text-slate-900">{{ __('subcon.subcounts.show.input_net') }}</div>
                                <p class="mt-1 text-xs text-slate-500">{{ __('subcon.subcounts.show.input_hint') }}</p>
                                <div class="mt-4">
                                    <label for="net_item_weight_kg" class="text-xs font-bold uppercase tracking-wider text-slate-500">{{ __('subcon.subcounts.show.net_kg') }}</label>
                                ...
                                @enderror
                                <button class="mt-4 w-full rounded-xl bg-slate-900 px-4 py-2.5 text-sm font-black text-white hover:bg-slate-800 disabled:opacity-60 disabled:cursor-not-allowed">{{ __('subcon.subcounts.show.save_net') }}</button>
                            </form>
                        </div>
                    </div>
                @empty
                    <div class="rounded-2xl border border-slate-200 bg-white p-10 text-center text-slate-400 shadow-sm">
                        {{ __('subcon.subcounts.show.empty_records') }}
                    </div>
                @endforelse
            </div>
        </div>
