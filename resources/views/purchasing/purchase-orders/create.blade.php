<x-app-layout>
    <x-slot name="header">
        {{ __('purchasing.orders.create.header') }}
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
            <div class="bg-white shadow-xl border border-slate-200 rounded-3xl overflow-hidden">
                <div class="p-8 border-b border-slate-100 bg-slate-50/50">
                    <h2 class="text-2xl font-black text-slate-900 tracking-tight">{{ __('purchasing.orders.create.title') }}</h2>
                    @if ($pr)
                        <p class="text-sm text-slate-500 mt-1">{{ __('purchasing.orders.create.from_pr_before') }} <span class="font-bold text-indigo-600">{{ $pr->pr_number }}</span> {{ __('purchasing.orders.create.from_pr_after') }}</p>
                    @else
                        <p class="text-sm text-slate-500 mt-1">{{ __('purchasing.orders.create.manual_hint') }}</p>
                    @endif
                </div>
                ...
                <form action="{{ route('purchasing.purchase-orders.store') }}" method="POST">
                    ...
                    <div class="p-8 space-y-8">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                            <div class="space-y-4">
                                <label for="vendor_id" class="block text-xs font-black text-slate-400 uppercase tracking-[0.2em] mb-1">{{ __('purchasing.orders.create.vendor_label') }}</label>
                                <select name="vendor_id" id="vendor_id" x-model="selectedVendorId" @change="onVendorChange()" class="w-full rounded-2xl border-slate-200 focus:ring-indigo-500 focus:border-indigo-500 text-sm font-semibold" required>
                                    <option value="">{{ __('purchasing.orders.create.choose_vendor') }}</option>
                            ...
                            <div class="space-y-4">
                                <label for="notes" class="block text-xs font-black text-slate-400 uppercase tracking-[0.2em] mb-1">{{ __('purchasing.orders.create.notes_label') }}</label>
                                <input type="text" name="notes" id="notes" class="w-full rounded-2xl border-slate-200 text-sm" placeholder="{{ __('purchasing.orders.create.notes_ph') }}">
                            </div>
                        </div>
                        ...
                        <div>
                            <h3 class="text-xs font-black text-slate-400 uppercase tracking-[0.2em] mb-4 flex items-center gap-2">
                                ...
                                {{ __('purchasing.orders.create.lines_title') }}
                            </h3>
                            <div class="overflow-hidden border border-slate-200 rounded-2xl">
                                <table class="min-w-full divide-y divide-slate-200">
                                    <thead class="bg-slate-50 text-xs text-slate-500 font-bold uppercase tracking-wider">
                                        <tr>
                                            <th class="px-6 py-4 text-left">{{ __('purchasing.orders.create.th_part') }}</th>
                                            <th class="px-6 py-4 text-right">{{ __('purchasing.orders.create.th_qty') }}</th>
                                            <th class="px-6 py-4 text-right">{{ __('purchasing.orders.create.th_price') }}</th>
                                            <th class="px-6 py-4 text-right">{{ __('purchasing.orders.create.th_subtotal') }}</th>
                                            @if (!$pr)
                                                <th class="px-6 py-4 text-center">{{ __('purchasing.orders.create.th_actions') }}</th>
                                            @endif
                                        </tr>
                                    </thead>
                                                    ...
                                                    <td class="px-6 py-4">
                                                        ...
                                                        <template x-if="getVendorPartInfo({{ $item->part_id }})">
                                                            <div class="mt-1 text-[10px] text-indigo-600 font-mono">
                                                                <span x-text='@js(__("purchasing.orders.create.vendor_part_prefix")) + getVendorPartInfo({{ $item->part_id }}).part_no'></span>
                                                            </div>
                                                        </template>
                                                        <template x-if="!getVendorPartInfo({{ $item->part_id }}) && selectedVendorId">
                                                            <div class="mt-1 text-[10px] text-amber-500 italic">{{ __('purchasing.orders.create.no_vendor_part') }}</div>
                                                        </template>
                                                ...
                                                <tr class="hover:bg-slate-50/50 transition-colors">
                                                    <td class="px-6 py-4">
                                                        <select :name="`items[${index}][part_id]`" class="w-full rounded-xl border-slate-200 text-sm" required>
                                                            <option value="">{{ __('purchasing.orders.create.select_part') }}</option>
                                                    ...
                                                    <td class="px-6 py-4 text-center">
                                                        <button type="button" @click="removeItem(index)" aria-label="{{ __('purchasing.orders.create.remove_item') }}" class="text-rose-500 hover:bg-rose-50 p-2 rounded-xl">
                                    ...
                                    <tfoot class="bg-slate-50/30">
                                        <tr>
                                            <td colspan="3" class="px-6 py-5 text-right text-xs font-black text-slate-400 uppercase tracking-widest">{{ __('purchasing.orders.create.grand_total') }}</td>
                            ...
                            @if (!$pr)
                                <button type="button" @click="addItem()" class="mt-4 px-4 py-2 rounded-xl border border-indigo-200 bg-indigo-50 text-indigo-700 font-bold hover:bg-indigo-100 transition-all flex items-center gap-2 text-xs uppercase tracking-widest shadow-sm">
                                    ...
                                    {{ __('purchasing.orders.create.add_item') }}
                                </button>
                            @endif
                        </div>
                    </div>
                    ...
                    <div class="p-8 border-t border-slate-100 bg-slate-50/50 flex items-center justify-end gap-3">
                        <a href="{{ route('purchasing.purchase-orders.index') }}" class="px-6 py-3 rounded-2xl bg-white border border-slate-200 text-slate-600 font-bold hover:bg-slate-50 transition-all uppercase text-xs tracking-wider">{{ __('purchasing.orders.create.discard') }}</a>
                        <button type="submit" class="px-10 py-3 rounded-2xl bg-slate-900 text-white font-bold hover:bg-slate-800 transition-all shadow-xl uppercase text-xs tracking-wider">{{ __('purchasing.orders.create.submit') }}</button>
                    </div>
                </form>
            </div>
        </div>
