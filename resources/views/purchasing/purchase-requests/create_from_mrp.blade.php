        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
            <div class="bg-white shadow-xl border border-slate-200 rounded-3xl overflow-hidden">
                <div class="p-6 border-b border-slate-100 bg-slate-50/50">
                    ...
                    <p class="text-sm text-slate-500 mt-1">Select items from the MRP Purchase Plan to create a formal Purchase Request.</p>
                </div>
                ...
                <form action="{{ route('purchasing.purchase-requests.store') }}" method="POST">
                                ...
                                <table class="min-w-full divide-y divide-slate-200">
                                    <thead class="bg-slate-50">
                                        <tr>
                                            ...
                                            <th class="px-4 py-3 text-right text-xs font-bold text-slate-500 uppercase tracking-widest">PR Qty</th>
                                        </tr>
                                    </thead>
                                    <tbody class="bg-white divide-y divide-slate-100">
                                        @forelse ($plans as $index => $plan)
                                            <tr class="hover:bg-slate-50/80 transition-colors">
                                                <td class="px-4 py-3">
                                                    <input type="checkbox" name="items[{{ $index }}][selected]" value="1" class="item-checkbox rounded border-slate-300 text-indigo-600 focus:ring-indigo-500">
                                                    <input type="hidden" name="items[{{ $index }}][part_id]" value="{{ $plan->part_id }}">
                                                </td>
                                                ...
                                                <td class="px-4 py-3 text-right text-xs font-bold text-indigo-600 font-mono">
                                                    {{ number_format($plan->planned_order_rec, 2) }}
                                                </td>
                                                <td class="px-4 py-3 text-right">
                                                    <input type="number" name="items[{{ $index }}][qty]" value="{{ (float)$plan->planned_order_rec }}" step="0.0001" class="w-24 rounded-xl border-slate-200 text-xs font-bold text-right" required>
                                                    <input type="hidden" name="items[{{ $index }}][required_date]" value="{{ $plan->plan_date }}">
                                                </td>
                                            </tr>
                                        @empty
                                            <tr>
                    ...
                    <div class="p-6 border-t border-slate-100 bg-slate-50/50 flex items-center justify-end gap-3">
                        ...
                        <button type="submit" class="px-8 py-2.5 rounded-2xl bg-indigo-600 text-white font-bold hover:bg-indigo-700 transition-all shadow-lg uppercase text-xs tracking-wider">Create Purchase Request</button>
                    </div>
                </form>
            </div>
        </div>
