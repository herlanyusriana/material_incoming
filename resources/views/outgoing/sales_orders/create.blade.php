<x-app-layout>
    <x-slot name="header">
        Outgoing • New Sales Order (PO Outgoing)
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
            <div class="bg-white shadow-xl border border-slate-200 rounded-3xl overflow-hidden">
                <div class="p-6 border-b border-slate-100 bg-slate-50/50">
                    <h2 class="text-xl font-bold text-slate-900 tracking-tight">Create Sales Order</h2>
                    <p class="text-sm text-slate-500 mt-1">Input new purchase order from customer.</p>
                </div>

                <form action="{{ route('outgoing.sales-orders.store') }}" method="POST">
                    @csrf
                    <div class="p-6">
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                            <div class="space-y-1">
                                <label class="text-xs font-bold text-slate-500 uppercase tracking-widest">SO
                                    Number</label>
                                <input type="text" name="so_no" value="{{ old('so_no') }}" required
                    ...
                    <div class="p-6 border-t border-slate-100 bg-slate-50/50 flex items-center justify-end gap-3">
                        ...
                        <button type="submit"
                            class="px-8 py-2.5 rounded-2xl bg-indigo-600 text-white font-bold hover:bg-indigo-700 transition-all shadow-lg uppercase text-xs tracking-wider">Save
                            Sales Order</button>
                    </div>
                </form>
            </div>
