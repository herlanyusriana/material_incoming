...
<div class="space-y-6">
		...
		<div class="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
			...
			<form method="GET" action="{{ route('outgoing.delivery-plan') }}" class="flex flex-wrap items-end gap-2">
				...
				<div>
					...
					<input type="number" step="0.01" min="0" name="uph_nr1" value="{{ (float) ($uphNr1 ?? 60) }}"
						class="rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm font-semibold text-slate-700 w-28 text-right">
				</div>
				<div>
					<div class="text-xs font-semibold text-slate-500 mb-1">UPH NR2</div>
					<input type="number" step="0.01" min="0" name="uph_nr2" value="{{ (float) ($uphNr2 ?? 60) }}"
						class="rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm font-semibold text-slate-700 w-28 text-right">
				</div>
				<button
		...
		<div class="grid grid-cols-1 xl:grid-cols-3 gap-6">
			...
			<div class="xl:col-span-1 bg-white rounded-2xl shadow-sm border border-slate-200 p-5">
				<div class="flex items-center justify-between mb-4">
					...
					<div class="text-[10px] font-bold text-slate-400">TOTAL UNIT:
						{{ number_format($totalsByLine->sum('balance')) }}</div>
				</div>
							...
							@foreach($totalsByLine as $t)
								<tr>
									...
									<td class="px-3 py-2 text-right font-bold text-indigo-600">
										{{ number_format((float) ($t->balance ?? 0), 0) }}</td>
									<td class="px-3 py-2 text-right text-slate-600">
										{{ number_format((float) ($t->hours ?? 0), 1) }}h</td>
									<td class="px-3 py-2 text-right font-semibold text-slate-700">{{ (int) ($t->jigs ?? 0) }}
									</td>
								</tr>
							@endforeach
						</tbody>
			...
			<div class="xl:col-span-2 bg-white rounded-2xl shadow-sm border border-slate-200 p-5">
				<div class="flex items-center justify-between mb-4">
					...
					<button onclick="document.getElementById('addTripModal').classList.remove('hidden')"
				...
				@if($unassignedSalesOrders->isNotEmpty())
					<div class="mb-6 p-4 bg-orange-50 border border-orange-100 rounded-xl">
						<div class="flex items-center gap-2 mb-3">
							<div class="px-2 py-0.5 rounded bg-orange-200 text-orange-700 text-[10px] font-black uppercase">
								Unassigned Sales Orders</div>
							<div class="text-[10px] text-orange-600 font-bold">{{ $unassignedSalesOrders->count() }} Orders
								Pending Trip Assignment</div>
						</div>
						<div class="flex flex-wrap gap-2">
							@foreach($unassignedSalesOrders as $so)
								<div
									class="bg-white border border-orange-200 rounded-lg p-3 shadow-sm flex flex-col gap-2 min-w-[200px]">
									<div class="flex justify-between items-start">
										<div>
											<div class="text-[11px] font-black text-slate-900">{{ $so->so_no }}</div>
											<div class="text-[10px] font-bold text-slate-500 uppercase">{{ $so->customer?->name }}
											</div>
										</div>
										<div class="text-[10px] font-black text-indigo-600 bg-indigo-50 px-1.5 py-0.5 rounded">
											{{ $so->items->count() }} SKU</div>
									</div>
									<select onchange="assignSoToTrip({{ $so->id }}, this.value)"
										class="text-[10px] font-bold border-slate-200 rounded-lg w-full bg-slate-50 focus:ring-indigo-500">
										<option value="">Move to Trip...</option>
										@foreach($plans as $p)
											<option value="{{ $p->id }}">Trip #{{ $p->sequence }}
												({{ $p->truck?->plate_no ?? 'No Truck' }})</option>
										@endforeach
									</select>
								</div>
							@endforeach
				...
				<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
					@forelse($plans as $plan)
						<div
							class="rounded-xl border border-slate-200 p-4 hover:border-indigo-200 transition-colors bg-slate-50/50">
							<div class="flex items-start justify-between">
								<div class="flex items-center gap-3">
									<div
										class="h-10 w-10 rounded-full bg-indigo-100 flex items-center justify-center text-indigo-700 font-black text-sm">
										#{{ $plan->sequence }}
									</div>
									<div>
										<div class="text-xs font-black text-slate-900">TRIP {{ $plan->sequence }}</div>
										<div class="text-[10px] font-bold text-slate-500 uppercase tracking-tighter">Status:
											{{ strtoupper($plan->status) }}</div>
									</div>
								</div>
								<div class="flex flex-col items-end gap-1">
									<span class="text-[10px] font-bold text-slate-400">{{ $plan->salesOrders->count() }} SOs
										Assigned</span>
								</div>
							</div>
							<div class="mt-4 space-y-2">
								<div class="flex items-center gap-2">
									...
									<div class="text-xs font-semibold text-slate-700">
										{{ $plan->truck?->plate_no ?? 'No Truck' }} <span
											class="text-[10px] font-normal text-slate-400">({{ $plan->truck?->type ?? '-' }})</span>
									</div>
								</div>
								<div class="flex items-center gap-2">
									<svg class="w-3.5 h-3.5 text-slate-400" fill="none" stroke="currentColor"
									...
									<div class="text-xs font-semibold text-slate-700">{{ $plan->driver?->name ?? 'No Driver' }}
									</div>
								</div>
							</div>
							<div class="mt-4 border-t border-slate-100 pt-3">
								...
								<div class="space-y-1.5">
									@foreach($plan->salesOrders as $so)
										<div
											class="flex items-center justify-between text-[11px] bg-white p-2 rounded border border-slate-100 shadow-sm">
											<div>
											...
											<div class="flex items-center gap-2">
												<div class="text-indigo-600 font-bold">
													{{ number_format($so->items->sum('qty_ordered')) }}</div>
												<button onclick="assignSoToTrip({{ $so->id }}, null)"
													class="text-slate-300 hover:text-red-500" title="Remove from Trip">✕</button>
											</div>
										</div>
									@endforeach
									@if($plan->salesOrders->isEmpty())
										<div class="text-[10px] italic text-slate-400 py-1 text-center">Empty trip. Assign SOs
											above.</div>
									@endif
								</div>
							</div>
							<div class="mt-4 pt-4 border-t border-slate-200 flex justify-end gap-2">
								<button
									onclick="openAssignResourcesModal({{ $plan->id }}, {{ $plan->truck_id ?? 'null' }}, {{ $plan->driver_id ?? 'null' }})"
									class="text-[10px] font-black text-indigo-600 hover:text-indigo-800 bg-white border border-slate-200 px-3 py-1.5 rounded-lg shadow-sm">
									EDIT TRIP
								</button>
							</div>
						</div>
					@empty
						<div
							class="md:col-span-2 py-8 text-center text-slate-400 italic text-xs bg-slate-50 rounded-xl border border-dashed border-slate-200">
	...
	@if(($autoMappedRowCount ?? 0) > 0)
		<div class="rounded-2xl border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-900">
			Auto-mapped <span class="font-bold">{{ (int) $autoMappedRowCount }}</span> row(s) ke FG berdasarkan Customer
			Part Mapping / Product Mapping.
		...
		<div class="rounded-2xl border border-amber-200 bg-amber-50 p-4">
			<div class="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
				<div class="text-sm text-amber-900">
					Ada <span class="font-bold">{{ count($unmappedRows) }}</span> row yang <span class="font-bold">belum
						terdaftar / belum bisa di-mapping</span> ke FG (gci_part_id kosong), sehingga tidak masuk ke
					Delivery Plan utama.
				</div>
				...
				<table class="min-w-full text-xs divide-y divide-amber-100">
					<thead class="bg-amber-50">
						<tr class="text-[11px] uppercase tracking-wider text-amber-900">
							...
							<th class="px-3 py-2 text-left font-bold w-40">Customer Part</th>
							<th class="px-3 py-2 text-right font-bold w-24">Total</th>
							<th class="px-3 py-2 text-center font-bold w-20">Action</th>
						</tr>
					</thead>
						...
						@foreach($unmappedRows as $u)
							<tr>
								...
								<td class="px-3 py-2 text-slate-600">
									@if(!empty($u->customer_part_no))
										<span class="text-blue-600">{{ $u->customer_part_no }}</span>
									@else
										<span class="text-slate-400 italic">-</span>
									@endif
								</td>
								<td class="px-3 py-2 text-right text-slate-900 font-bold">
									{{ number_format((float) ($u->total_qty ?? 0), 0) }}</td>
								<td class="px-3 py-2 text-center">
									<a href="{{ route('planning.customer-parts.index', ['search' => $u->part_no ?? '']) }}"
										target="_blank"
										class="inline-flex items-center gap-1 px-2 py-1 text-xs font-bold text-indigo-600 bg-indigo-50 rounded hover:bg-indigo-100 transition-colors">
		...
		<div class="dp-scroll">
			<table class="dp-table min-w-max w-full text-xs">
				<thead class="bg-slate-50 border-b border-slate-200">
					<tr>
						<th rowspan="2"
							class="px-2 py-2 h-10 text-left font-bold text-slate-700 border-r border-slate-200 w-12 sticky-l left-0">
							No</th>
						<th rowspan="2"
							class="px-2 py-2 h-10 text-left font-bold text-slate-700 border-r border-slate-200 w-[120px] sticky-l left-12">
							Classification</th>
						<th rowspan="2"
							class="px-2 py-2 h-10 text-left font-bold text-slate-700 border-r border-slate-200 w-[220px] sticky-l left-[168px]">
							Model</th>
						<th rowspan="2"
							class="px-2 py-2 h-10 text-left font-bold text-slate-700 border-r border-slate-200 w-[140px] sticky-l left-[388px]">
							Part Number</th>
						<th rowspan="2"
							class="px-2 py-2 h-10 text-center font-bold text-slate-700 border-r border-slate-200 w-[90px] sticky-l left-[528px]">
							Std Pack</th>
						<th rowspan="2"
							class="px-2 py-2 h-10 text-right font-bold text-slate-700 border-r border-slate-200 w-[90px] sticky-l left-[618px]">
							Plan</th>
						<th rowspan="2"
							class="px-2 py-2 h-10 text-right font-bold text-slate-700 border-r border-slate-200 w-[130px] sticky-l left-[708px]">
							Stock at Customer</th>
						<th rowspan="2"
							class="px-2 py-2 h-10 text-right font-bold text-slate-700 border-r border-slate-200 w-[90px] sticky-l left-[838px]">
							Balance</th>
						<th rowspan="2"
							class="px-2 py-2 h-10 text-left font-bold text-slate-700 border-r border-slate-200 w-[110px] sticky-l left-[928px]">
							Duedate</th>
						<th rowspan="2"
							class="px-2 py-2 h-10 text-left font-bold text-slate-700 border-r border-slate-200 w-[90px]">
							Line</th>
						<th rowspan="2"
							class="px-2 py-2 h-10 text-right font-bold text-slate-700 border-r border-slate-200 w-[90px]">
							UPH</th>
						<th rowspan="2"
							class="px-2 py-2 h-10 text-right font-bold text-slate-700 border-r border-slate-200 w-[90px]">
							Hours</th>
						<th rowspan="2"
							class="px-2 py-2 h-10 text-right font-bold text-slate-700 w-[90px] sticky-r right-[110px] border-l border-slate-200">
							JIG</th>
						<th rowspan="2"
							class="px-2 py-2 h-10 text-right font-bold text-slate-700 w-[110px] sticky-r right-0 border-l border-slate-200">
							Action</th>
					</tr>
				</thead>
				<tbody class="divide-y divide-slate-100">
					...
					<tr class="hover:bg-slate-50">
						<td class="px-2 py-2 border-r border-slate-200 text-slate-600 font-semibold sticky-l left-0">
							{{ $no }}</td>
						<td class="px-2 py-2 border-r border-slate-200 text-slate-700 sticky-l left-12">
							{{ $r->delivery_class }}</td>
						<td
							class="px-2 py-2 border-r border-slate-200 text-slate-700 sticky-l left-[168px] font-semibold">
							{{ $r->part_model ?: '-' }}
						</td>
						<td
							class="px-2 py-2 border-r border-slate-200 font-mono text-indigo-700 font-bold sticky-l left-[388px]">
							{{ $r->part_no }}</td>
						<td
							class="px-2 py-2 border-r border-slate-200 text-center text-slate-700 font-semibold sticky-l left-[528px]">
						...
						<td
							class="px-2 py-2 border-r border-slate-200 text-right font-bold text-slate-900 sticky-l left-[618px]">
							{{ number_format((float) $r->plan_total, 0) }}</td>
						<td class="px-2 py-2 border-r border-slate-200 text-right text-slate-700 sticky-l left-[708px]">
							{{ (float) $r->stock_at_customer > 0 ? number_format((float) $r->stock_at_customer, 0) : '-' }}
						</td>
						<td class="px-2 py-2 border-r border-slate-200 text-right sticky-l left-[838px]">
							...
							@if(!empty($r->has_overrides))
								<div
									class="mt-1 inline-flex items-center rounded bg-amber-100 px-2 py-0.5 text-[10px] font-bold text-amber-900">
									OVERRIDE
								</div>
							@endif
						</td>
						<td class="px-2 py-2 border-r border-slate-200 text-slate-700 sticky-l left-[928px]">
							...
							@if(!empty($r->auto_mapped))
								<div
									class="mt-1 inline-flex items-center rounded bg-emerald-100 px-2 py-0.5 text-[10px] font-bold text-emerald-800">
									AUTO-MAPPED
								</div>
							@endif
						</td>
						...
						<td class="px-2 py-2 border-r border-slate-200 text-right text-slate-700">
							{{ number_format((float) ($r->uph ?? 0), 2) }}</td>
						<td class="px-2 py-2 border-r border-slate-200 text-right text-slate-700">
							{{ number_format((float) ($r->hours ?? 0), 2) }}</td>
						<td
							class="px-2 py-2 text-right font-bold text-slate-900 sticky-r right-[110px] border-l border-slate-200">
							{{ (int) ($r->jigs ?? 0) }}</td>
						<td class="px-2 py-2 text-right sticky-r right-0 border-l border-slate-200">
							<div class="flex flex-col gap-2 items-end">
								<button type="button"
									...
									onclick="openOverrideModal(this)" data-date="{{ $selectedDate }}"
									data-gci-part-id="{{ (int) ($r->gci_part_id ?? 0) }}"
									data-part-no="{{ $r->part_no }}" data-part-name="{{ $r->part_name }}"
									...
									data-ov-notes="{{ (string) ($r->assignment?->notes ?? '') }}" @disabled(((int) ($r->gci_part_id ?? 0)) <= 0)>
									Edit
								</button>
								<button type="button"
									...
									onclick="openCreateSoModal(this)" data-date="{{ $selectedDate }}"
									data-customer-id="{{ (int) ($r->customer_id ?? 0) }}"
									data-gci-part-id="{{ (int) ($r->gci_part_id ?? 0) }}"
									data-part-no="{{ $r->part_no }}" data-part-name="{{ $r->part_name }}"
									data-balance="{{ (float) ($r->balance ?? 0) }}" @disabled(((int) ($r->customer_id ?? 0)) <= 0 || ((int) ($r->gci_part_id ?? 0)) <= 0)>
									Create SO
								</button>
							</div>
						</td>
		...
		<div class="bg-white rounded-2xl shadow-xl border border-slate-200 overflow-hidden">
			<div class="px-6 py-4 border-b border-slate-200 bg-slate-50 flex items-center justify-between">
				...
				<button type="button" class="text-slate-500 hover:text-slate-900"
					onclick="closeCreateSoModal()">✕</button>
			</div>
			<form id="createSoForm" method="POST" action="{{ route('outgoing.generate-so') }}" class="p-6 space-y-4">
				...
				<div>
					...
					<input type="number" step="0.0001" min="0.0001" name="items[0][qty]" id="so_qty"
						class="w-full rounded-lg border-slate-300 text-sm text-right" required>
				</div>

				<div class="flex justify-end gap-2 pt-2">
					<button type="button"
						class="rounded-lg border border-slate-200 bg-white px-4 py-2 text-sm font-bold text-slate-700 hover:bg-slate-50"
						onclick="closeCreateSoModal()">Cancel</button>
					<button
						class="rounded-lg bg-indigo-600 px-4 py-2 text-sm font-bold text-white hover:bg-indigo-700">Create</button>
				</div>
			</form>
		</div>
	</div>
	...
	<div class="absolute inset-x-0 top-12 mx-auto w-full max-w-xl px-4">
		<div class="bg-white rounded-2xl shadow-xl border border-slate-200 overflow-hidden">
			<div class="px-6 py-4 border-b border-slate-200 bg-slate-50 flex items-center justify-between">
				...
				<button type="button" class="text-slate-500 hover:text-slate-900"
					onclick="closeOverrideModal()">✕</button>
			</div>

			<form id="overrideForm" method="POST" action="{{ route('outgoing.delivery-plan.update-overrides') }}"
				class="p-6 space-y-4">
				@csrf
				...
				<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
					<div>
						...
						<select name="line_type_override" id="ov_line_type"
					...
					<div>
						...
						<input type="text" maxlength="255" name="notes" id="ov_notes"
							class="w-full rounded-lg border-slate-300 text-sm" placeholder="Optional note">
					</div>
				</div>
				...
				<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
					<div class="rounded-xl border border-slate-200 p-4">
						...
						<div class="mt-3 grid grid-cols-2 gap-3">
							<div>
								...
								<input type="number" min="1" step="1" name="jig_capacity_nr1_override" id="ov_cap_nr1"
									class="w-full rounded-lg border-slate-300 text-sm text-right" placeholder="(Auto)">
							</div>
							<div>
								<div class="text-xs font-semibold text-slate-500 mb-1">UPH</div>
								<input type="number" min="0.01" step="0.01" name="uph_nr1_override" id="ov_uph_nr1"
									class="w-full rounded-lg border-slate-300 text-sm text-right" placeholder="(Auto)">
							</div>
						</div>
					</div>
					...
					<div class="rounded-xl border border-slate-200 p-4">
						...
						<div class="mt-3 grid grid-cols-2 gap-3">
							<div>
								<div class="text-xs font-semibold text-slate-500 mb-1">JIG Capacity</div>
								<input type="number" min="1" step="1" name="jig_capacity_nr2_override" id="ov_cap_nr2"
									class="w-full rounded-lg border-slate-300 text-sm text-right" placeholder="(Auto)">
							</div>
							<div>
								<div class="text-xs font-semibold text-slate-500 mb-1">UPH</div>
								<input type="number" min="0.01" step="0.01" name="uph_nr2_override" id="ov_uph_nr2"
									class="w-full rounded-lg border-slate-300 text-sm text-right" placeholder="(Auto)">
							</div>
						</div>
					</div>
				...
				<div class="flex items-center justify-between gap-2 pt-2">
					<button type="button"
						class="rounded-lg border border-slate-200 bg-white px-4 py-2 text-sm font-bold text-slate-700 hover:bg-slate-50"
						onclick="resetOverrideForm()">Reset Overrides</button>
					<div class="flex gap-2">
						<button type="button"
							class="rounded-lg border border-slate-200 bg-white px-4 py-2 text-sm font-bold text-slate-700 hover:bg-slate-50"
							onclick="closeOverrideModal()">Cancel</button>
						<button
							class="rounded-lg bg-slate-900 px-4 py-2 text-sm font-bold text-white hover:bg-slate-800">Save</button>
					</div>
				</div>
			</form>
		</div>
...
<div id="addTripModal" class="hidden fixed inset-0 z-50">
	<div class="absolute inset-0 bg-slate-900/40"
		onclick="document.getElementById('addTripModal').classList.add('hidden')"></div>
	<div class="absolute inset-x-0 top-20 mx-auto w-full max-w-lg px-4">
		<div class="bg-white rounded-2xl shadow-xl border border-slate-200 overflow-hidden">
			<div class="px-6 py-4 border-b border-slate-200 bg-slate-50 flex items-center justify-between">
				<div class="font-black text-slate-900 uppercase">Add New Trip</div>
				<button type="button" class="text-slate-500 hover:text-slate-900"
					onclick="document.getElementById('addTripModal').classList.add('hidden')">✕</button>
			</div>
			<form method="POST" action="{{ route('outgoing.delivery-plan.store') }}" class="p-6 space-y-4">
				...
				<div class="flex justify-end gap-2 pt-2">
					<button type="button"
						...
						onclick="document.getElementById('addTripModal').classList.add('hidden')">Cancel</button>
					<button
						class="rounded-xl bg-slate-900 px-4 py-2 text-sm font-bold text-white shadow-lg shadow-slate-200">Create
						Trip</button>
				</div>
			</form>
		</div>
...
<div id="assignResourcesModal" class="hidden fixed inset-0 z-50">
	<div class="absolute inset-0 bg-slate-900/40"
		onclick="document.getElementById('assignResourcesModal').classList.add('hidden')"></div>
	<div class="absolute inset-x-0 top-20 mx-auto w-full max-w-lg px-4">
		<div class="bg-white rounded-2xl shadow-xl border border-slate-200 overflow-hidden">
			<div class="px-6 py-4 border-b border-slate-200 bg-slate-50 flex items-center justify-between">
				<div class="font-black text-slate-900 uppercase">Edit Trip Resources</div>
				<button type="button" class="text-slate-500 hover:text-slate-900"
					onclick="document.getElementById('assignResourcesModal').classList.add('hidden')">✕</button>
			</div>
			<form id="assignResourcesForm" method="POST" action="" class="p-6 space-y-4">
				...
				<div class="flex justify-end gap-2 pt-2">
					<button type="button"
						class="rounded-xl border border-slate-200 bg-white px-4 py-2 text-sm font-bold text-slate-700"
						onclick="document.getElementById('assignResourcesModal').classList.add('hidden')">Cancel</button>
					<button
						class="rounded-xl bg-indigo-600 px-4 py-2 text-sm font-bold text-white shadow-lg shadow-indigo-100">Save
						Changes</button>
				</div>
			</form>
		</div>
