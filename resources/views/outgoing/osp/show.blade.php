...
@section('content')
    <div class="space-y-6">
        ...
        <div class="rounded-2xl border border-indigo-200 bg-indigo-50 px-5 py-4 text-sm text-indigo-900">
            Dokumen ini dipakai saat part FG yang sama keluar melalui jalur OSP. Jadi part tetap sama, yang berbeda adalah flow dan dokumennya.
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 text-sm">
                ...
                <div>
                    <div class="text-xs font-bold uppercase tracking-wider text-slate-500">Document Date</div>
                    <div class="mt-1 text-slate-900">{{ $ospOrder->received_date->format('d/m/Y') }}</div>
                </div>
                <div>
                    <div class="text-xs font-bold uppercase tracking-wider text-slate-500">Target Outgoing Date</div>
        ...
        @if (!in_array($ospOrder->status, ['shipped', 'cancelled']))
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                {{-- Assembly Progress --}}
                <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
                    <h2 class="text-lg font-bold text-slate-900 mb-4">Update OSP Progress</h2>
                ...
                {{-- Ship --}}
                <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
                    <h2 class="text-lg font-bold text-slate-900 mb-4">Finalize OSP Outgoing</h2>
