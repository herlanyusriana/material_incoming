        ...
        <div id="where-used" class="bg-white shadow-sm border border-slate-200 rounded-2xl p-6">
            <div class="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
                <div>
                    ...
                    <p class="text-sm text-slate-600">Cari 1 component part number → ketahuan dipakai di FG mana + customer product mana.</p>
                </div>
            ...
            <div class="mt-4 flex gap-3">
                <div class="flex-1">
                    <input
                        ...
                        x-model="searchQuery"
                        @keyup.enter="search()"
                        class="w-full rounded-xl border-slate-200 text-base"
                        placeholder="Enter part number (e.g., RM-001)"
                    >
                </div>
                <button
                    type="button"
        ...
        <div x-show="hasSearched" x-cloak class="space-y-4">
            <div x-show="results.length === 0 && !isLoading" class="bg-yellow-50 border border-yellow-200 rounded-2xl p-8 text-center">
                ...
                <p class="text-yellow-700">
                    Component <span class="font-mono font-bold" x-text="lastSearchQuery"></span> tidak dipakai di BOM mana pun.
                </p>
            </div>
                        ...
                        <tbody class="divide-y divide-slate-100">
                            <template x-for="bom in results" :key="bom.id">
                                <tr class="hover:bg-slate-50">
                                    <td class="px-4 py-3 font-mono font-semibold text-slate-800" x-text="bom.fg_part_no"></td>
                                    ...
                                    <td class="px-4 py-3">
                                        ...
                                        <template x-if="(bom.customer_products || []).length > 0">
                                            <div class="space-y-2">
                                                <template x-for="cp in bom.customer_products" :key="cp.customer_part_no + '|' + cp.customer_name">
                                                    <div class="rounded-lg border border-slate-200 bg-white p-2">
                                                        <div class="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
                                                            ...
                                                            <div class="text-xs text-slate-600">
                                                                <span class="font-semibold" x-text="cp.customer_name"></span>
