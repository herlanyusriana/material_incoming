    ...
    <form method="POST" action="{{ route('delivery.outgoing.update', $deliveryNote) }}" class="bg-white rounded-xl border border-slate-200 shadow-sm p-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
            <div>
                ...
                <select name="customer_id" id="customer_id" required class="w-full rounded-lg border-slate-200 bg-slate-50 focus:border-indigo-500 focus:ring-indigo-500" disabled>
            ...
            <div>
                <label for="truck_id" class="block text-sm font-medium text-slate-700 mb-1">Truck</label>
            ...
            <div>
                ...
                <select name="status" id="status" required class="w-full rounded-lg border-slate-200 bg-slate-50 focus:border-indigo-500 focus:ring-indigo-500">
            ...
            <div>
                <label for="delivery_date" class="block text-sm font-medium text-slate-700 mb-1">Delivery Date</label>
                <input type="date" name="delivery_date" id="delivery_date" value="{{ old('delivery_date', $deliveryNote->delivery_date) }}"
            ...
            <div class="md:col-span-2">
                ...
                <textarea name="notes" id="notes" rows="3" class="w-full rounded-lg border-slate-200 bg-slate-50 focus:border-indigo-500 focus:ring-indigo-500">{{ old('notes', $deliveryNote->notes) }}</textarea>
                @error('notes')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
