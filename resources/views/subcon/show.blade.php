...
@section('content')
    ...
    <div class="space-y-6">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <div class="mt-1 text-sm text-slate-600">{{ __('subcon.orders.show.detail') }}</div>
                </div>
                <div class="flex gap-2 items-center">
                    ...
                    <span class="inline-flex items-center rounded-full px-3 py-1 text-sm font-bold {{ $statusColors[$subconOrder->status] ?? '' }}">
                        {{ __('subcon.status.'.$subconOrder->status) }}
                    </span>
                    <a href="{{ route('subcon.index') }}" class="text-sm text-slate-500 hover:text-slate-800">&larr; {{ __('subcon.orders.show.back') }}</a>
                </div>
            </div>
        </div>
        ...
        {{-- Order Details --}}
        <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
            <h2 class="text-lg font-bold text-slate-900 mb-4">{{ __('subcon.orders.show.order_info') }}</h2>
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 text-sm">
                <div>
                    <div class="text-xs font-bold uppercase tracking-wider text-slate-500">{{ __('subcon.orders.show.contract_no') }}</div>
                    <div class="mt-1 text-slate-900 font-semibold">{{ $subconOrder->contract_no ?? '-' }}</div>
                </div>
                <div>
                    <div class="text-xs font-bold uppercase tracking-wider text-slate-500">{{ __('subcon.orders.show.vendor') }}</div>
                    <div class="mt-1 text-slate-900 font-semibold">{{ $subconOrder->vendor->vendor_name ?? '-' }}</div>
                </div>
                <div>
                    <div class="text-xs font-bold uppercase tracking-wider text-slate-500">{{ __('subcon.orders.show.rm_part') }}</div>
                ...
                <div>
                    <div class="text-xs font-bold uppercase tracking-wider text-slate-500">{{ __('subcon.orders.show.wip_part') }}</div>
                ...
                <div>
                    <div class="text-xs font-bold uppercase tracking-wider text-slate-500">{{ __('subcon.orders.show.process') }}</div>
                    <div class="mt-1 text-slate-900 font-semibold">{{ $subconOrder->process_type }}</div>
                </div>
                <div>
                    <div class="text-xs font-bold uppercase tracking-wider text-slate-500">{{ __('subcon.orders.show.sent_date') }}</div>
                    <div class="mt-1 text-slate-900">{{ $subconOrder->sent_date->format('d/m/Y') }}</div>
                </div>
                <div>
                    <div class="text-xs font-bold uppercase tracking-wider text-slate-500">{{ __('subcon.orders.show.expected_return') }}</div>
                    <div class="mt-1 text-slate-900">{{ $subconOrder->expected_return_date?->format('d/m/Y') ?? '-' }}</div>
                </div>
                <div>
                    <div class="text-xs font-bold uppercase tracking-wider text-slate-500">{{ __('subcon.orders.show.created_by') }}</div>
                    <div class="mt-1 text-slate-900">{{ $subconOrder->creator->name ?? '-' }}</div>
                </div>
                <div>
                    <div class="text-xs font-bold uppercase tracking-wider text-slate-500">{{ __('subcon.orders.show.stock_sent_posted') }}</div>
                    <div class="mt-1 text-slate-900">
                        {{ $subconOrder->sent_posted_at?->format('d/m/Y H:i') ?? '-' }}
                        @if($subconOrder->sender)
                            <span class="text-slate-500">{{ __('subcon.orders.show.by') }} {{ $subconOrder->sender->name }}</span>
                        @endif
                    </div>
                </div>
            </div>
            ...
            <div class="mt-6 pt-6 border-t border-slate-200">
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div class="rounded-lg bg-blue-50 p-4 text-center">
                        <div class="text-xs font-bold text-blue-600 uppercase">{{ __('subcon.orders.show.qty_sent') }}</div>
                    ...
                    <div class="rounded-lg bg-emerald-50 p-4 text-center">
                        <div class="text-xs font-bold text-emerald-600 uppercase">{{ __('subcon.orders.show.qty_received') }}</div>
                    ...
                    <div class="rounded-lg bg-red-50 p-4 text-center">
                        <div class="text-xs font-bold text-red-600 uppercase">{{ __('subcon.orders.show.qty_rejected') }}</div>
                    ...
                    <div class="rounded-lg bg-amber-50 p-4 text-center">
                        <div class="text-xs font-bold text-amber-600 uppercase">{{ __('subcon.orders.show.outstanding') }}</div>
            ...
            @if ($subconOrder->notes)
                <div class="mt-4 pt-4 border-t border-slate-200">
                    <div class="text-xs font-bold uppercase tracking-wider text-slate-500">{{ __('subcon.orders.show.notes') }}</div>
        ...
        @if (!in_array($subconOrder->status, ['completed', 'cancelled']))
            <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
                <h2 class="text-lg font-bold text-slate-900 mb-4">{{ __('subcon.orders.show.record_receive') }}</h2>
                <form action="{{ route('subcon.receive', $subconOrder) }}" method="POST" enctype="multipart/form-data" class="space-y-4" onsubmit="return confirmSubconNgReceive(this);">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                        <div>
                            <label for="qty_good" class="block text-sm font-bold text-slate-700 mb-1">{{ __('subcon.orders.show.qty_good') }} <span class="text-red-500">*</span></label>
                        ...
                        <div>
                            <label for="weight_kgm" class="block text-sm font-bold text-slate-700 mb-1">{{ __('subcon.orders.show.weight_good') }} <span class="text-red-500">*</span></label>
                            <input type="number" id="weight_kgm" step="0.0001" min="0" name="weight_kgm" value="{{ old('weight_kgm') }}"
                                class="w-full rounded-lg border-emerald-300 text-sm focus:border-emerald-500 focus:ring-emerald-500" required />
                        </div>
                        <div>
                            <label for="qty_rejected" class="block text-sm font-bold text-slate-700 mb-1">{{ __('subcon.orders.show.qty_rejected_label') }}</label>
                            ...
                            <div class="mt-1 text-xs font-semibold text-rose-600">{{ __('subcon.orders.show.ng_hint') }}</div>
                        </div>
                        <div>
                            <label for="weight_rejected_kgm" class="block text-sm font-bold text-slate-700 mb-1">{{ __('subcon.orders.show.weight_rejected') }}</label>
                            <input type="number" id="weight_rejected_kgm" step="0.0001" min="0" name="weight_rejected_kgm" value="{{ old('weight_rejected_kgm') }}"
                                class="w-full rounded-lg border-rose-300 text-sm focus:border-rose-500 focus:ring-rose-500" />
                        </div>
                        <div>
                            <label for="received_date" class="block text-sm font-bold text-slate-700 mb-1">{{ __('subcon.orders.show.received_date') }} <span class="text-red-500">*</span></label>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <label for="sj_file" class="block text-sm font-bold text-slate-700 mb-1">{{ __('subcon.orders.show.upload_sj') }}</label>
                        ...
                        <div>
                            <label for="invoice_file" class="block text-sm font-bold text-slate-700 mb-1">{{ __('subcon.orders.show.upload_invoice') }}</label>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <label for="receive_location_code" class="block text-sm font-bold text-slate-700 mb-1">{{ __('subcon.orders.show.wh_receive_location') }}</label>
                            ...
                            <div class="mt-1 text-xs text-slate-500">{{ __('subcon.orders.show.wh_receive_hint') }}</div>
                        </div>
                        <div>
                            <label for="reject_location_code" class="block text-sm font-bold text-slate-700 mb-1">{{ __('subcon.orders.show.wh_reject_location') }}</label>
                            <input type="text" id="reject_location_code" name="reject_location_code" value="{{ old('reject_location_code') }}"
                                class="w-full rounded-lg border-slate-300 text-sm focus:border-indigo-500 focus:ring-indigo-500" />
                            <div class="mt-1 text-xs text-slate-500">{{ __('subcon.orders.show.wh_reject_hint') }}</div>
                        </div>
                    </div>
                    <div>
                        <label for="notes" class="block text-sm font-bold text-slate-700 mb-1">{{ __('subcon.orders.show.notes') }}</label>
                        <textarea id="notes" name="notes" rows="2"
                            class="w-full rounded-lg border-slate-300 text-sm focus:border-indigo-500 focus:ring-indigo-500"
                            placeholder="{{ __('subcon.orders.show.notes_optional') }}"></textarea>
                    </div>
                    <button type="submit"
                        ...
                        {{ __('subcon.orders.show.submit_receive') }}
                    </button>
                </form>
            </div>
        @endif
        ...
        @if ($subconOrder->receives->isNotEmpty())
            <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
                <div class="px-6 py-4 border-b border-slate-200">
                    <h2 class="text-lg font-bold text-slate-900">{{ __('subcon.orders.show.receive_history') }}</h2>
                </div>
                <table class="w-full text-sm divide-y divide-slate-200">
                    <thead class="bg-slate-50">
                        <tr>
                            <th class="px-4 py-3 text-left font-bold text-slate-700">#</th>
                            <th class="px-4 py-3 text-center font-bold text-slate-700"{{ __('subcon.orders.show.date') }}</th>
                            <th class="px-4 py-3 text-right font-bold text-slate-700"{{ __('subcon.orders.show.qty_good') }}</th>
                            <th class="px-4 py-3 text-right font-bold text-slate-700"{{ __('subcon.orders.show.qty_rejected_label') }}</th>
                            <th class="px-4 py-3 text-left font-bold text-slate-700"{{ __('subcon.orders.show.wh_good') }}</th>
                            <th class="px-4 py-3 text-left font-bold text-slate-700"{{ __('subcon.orders.show.wh_reject') }}</th>
                            <th class="px-4 py-3 text-left font-bold text-slate-700"{{ __('subcon.orders.show.notes') }}</th>
                            <th class="px-4 py-3 text-left font-bold text-slate-700"{{ __('subcon.orders.show.by_col') }}</th>
                        </tr>
                    </thead>
                        ...
                        @foreach ($subconOrder->receives as $i => $rec)
                            <tr class="hover:bg-slate-50">
                                ...
                                <td class="px-4 py-3 text-slate-600">
                                    <div class="font-semibold">{{ ($rec->receive_location_code ?? '') === 'WIP-BYPASS' ? __('subcon.orders.show.no_location') : ($rec->receive_location_code ?? '-') }}</div>
                                ...
                                <td class="px-4 py-3 text-slate-600">
                                    <div class="flex flex-col gap-1">
                                        ...
                                        <div class="flex gap-2">
                                            @if($rec->sj_file_path)
                                                <a href="{{ Storage::url($rec->sj_file_path) }}" target="_blank" class="text-xs text-indigo-600 hover:underline">{{ __('subcon.orders.show.view_sj') }}</a>
                                            @endif
                                            @if($rec->invoice_file_path)
                                                <a href="{{ Storage::url($rec->invoice_file_path) }}" target="_blank" class="text-xs text-indigo-600 hover:underline">{{ __('subcon.orders.show.view_invoice') }}</a>
                                            @endif
                                        </div>
                                    </div>
                                </td>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-rose-200 overflow-hidden">
            <div class="px-6 py-4 border-b border-rose-100 bg-rose-50/60">
                <div class="flex items-center justify-between gap-4">
                    <div>
                        <h2 class="text-lg font-bold text-rose-950">{{ __('subcon.orders.show.reject_history') }}</h2>
                        <div class="mt-1 text-sm text-rose-700">{{ __('subcon.orders.show.reject_desc') }}</div>
                    </div>
                    <div class="text-right">
                        <div class="text-xs font-black uppercase tracking-wider text-rose-600">{{ __('subcon.orders.show.total_reject') }}</div>
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-sm divide-y divide-slate-200">
                    <thead class="bg-slate-50">
                        <tr>
                            <th class="px-4 py-3 text-left font-bold text-slate-700">#</th>
                            <th class="px-4 py-3 text-center font-bold text-slate-700"{{ __('subcon.orders.show.receive_date') }}</th>
                            <th class="px-4 py-3 text-right font-bold text-slate-700"{{ __('subcon.orders.show.qty_reject') }}</th>
                            <th class="px-4 py-3 text-right font-bold text-slate-700"{{ __('subcon.orders.show.weight_kgm') }}</th>
                            <th class="px-4 py-3 text-left font-bold text-slate-700"{{ __('subcon.orders.show.reject_location') }}</th>
                            <th class="px-4 py-3 text-left font-bold text-slate-700"{{ __('subcon.orders.show.posted_wh') }}</th>
                            <th class="px-4 py-3 text-left font-bold text-slate-700"{{ __('subcon.orders.show.notes_docs') }}</th>
                            <th class="px-4 py-3 text-left font-bold text-slate-700"{{ __('subcon.orders.show.by_col') }}</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-slate-100">
                        ...
                        @empty
                            <tr>
                                <td colspan="8" class="px-4 py-8 text-center text-slate-400">{{ __('subcon.orders.show.empty_reject') }}</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
        ...
        @if(isset($traceability) && $traceability->isNotEmpty())
            <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
                <div class="px-6 py-4 border-b border-slate-200">
                    <h2 class="text-lg font-bold text-slate-900">{{ __('subcon.orders.show.traceability') }}</h2>
                </div>
                <table class="w-full text-sm divide-y divide-slate-200">
                    <thead class="bg-slate-50">
                        <tr>
                            <th class="px-4 py-3 text-left font-bold text-slate-700">{{ __('subcon.orders.show.contract_no') }}</th>
                            <th class="px-4 py-3 text-left font-bold text-slate-700"{{ __('subcon.orders.show.time') }}</th>
                            <th class="px-4 py-3 text-left font-bold text-slate-700"{{ __('subcon.orders.show.type') }}</th>
                            <th class="px-4 py-3 text-left font-bold text-slate-700"{{ __('subcon.orders.show.location') }}</th>
                            <th class="px-4 py-3 text-right font-bold text-slate-700"{{ __('subcon.orders.show.qty_change') }}</th>
                            <th class="px-4 py-3 text-left font-bold text-slate-700"{{ __('subcon.orders.show.by_col') }}</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-slate-100">
                        @foreach ($traceability as $move)
                            <tr class="hover:bg-slate-50">
                                ...
                                <td class="px-4 py-3 text-slate-700">{{ ($move->location_code ?? '') === 'WIP-BYPASS' ? __('subcon.orders.show.no_stock_cut') : ($move->location_code ?? '-') }}</td>
        ...
        @if (!in_array($subconOrder->status, ['completed', 'cancelled']))
            <div class="flex justify-end">
                <form action="{{ route('subcon.cancel', $subconOrder) }}" method="POST"
                    onsubmit='return confirm(@js(__('subcon.orders.show.confirm_cancel')));'>
                    @csrf
                    <button type="submit"
                        ...
                        {{ __('subcon.orders.show.cancel_order') }}
                    </button>
                </form>
            </div>
        @endif
    ...
    <script>
        function confirmSubconNgReceive(form) {
            ...
            return confirm(@js(__('subcon.orders.show.confirm_ng_receive', ['qty' => '__QTY__'])).replace('__QTY__', rejectedQty));
        }
    </script>
@endsection
