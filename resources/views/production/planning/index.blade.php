                                ...
                                <tr x-show="matchesRow({{ $loop->index }})" class="group hover:bg-blue-50/35">
                                    ...
                                    <td class="px-3 py-3 align-middle">
                                        <div class="flex items-center justify-center gap-1">
                                            ...
                                            <button type="button" @click="generateWo({{ $part?->id }}, {{ (float) ($row['pending_wo_qty'] ?? 0) }})" aria-label="Buat WO {{ $part?->part_no }}" title="Buat WO + alokasi material (sisa target yang belum dibuat)" class="inline-flex h-10 w-10 items-center justify-center rounded-lg text-blue-700 transition hover:bg-blue-100 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-600"><svg aria-hidden="true" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" d="M12 5v14M5 12h14" /></svg></button>
        ...
        <script>
            function productionPlanBoard(config) {
                return {
                    ...
                    async generateWo(partId, qty) { try { await this.waitForSaves(); } catch (_) { return; } if (!(Number(qty) > 0)) { this.notify('Target harian part ini masih 0 — isi qty dulu.', 'error'); return; } window.location = @js(route('production.wo-tracking.create')) + '?gci_part_id=' + encodeURIComponent(partId) + '&start_date=' + encodeURIComponent(this.startDate) + '&qty_target=' + encodeURIComponent(qty); },
