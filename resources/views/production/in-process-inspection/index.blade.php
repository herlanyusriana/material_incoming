<x-app-layout>
    ...
    <div class="space-y-6">
        <div class="flex justify-between items-center">
            <h2 class="text-xl font-semibold text-slate-800">In-Process Inspections</h2>
        </div>
        ...
        <form method="GET" class="bg-white border rounded-xl shadow-sm p-4">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-3 items-end">
                <div>
                    <label class="block text-xs font-semibold text-slate-600" for="ipi-status">Status</label>
                    <select name="status" id="ipi-status" class="mt-1 w-full rounded-lg border-slate-200 text-sm">
                ...
                <div class="flex gap-2">
                    <a href="{{ route('production.in-process-inspection.index') }}" class="px-3 py-2 rounded-lg border border-slate-200 text-sm font-semibold text-slate-700 hover:bg-slate-50">Reset</a>
                    ...
                    @forelse($inspections as $inspection)
                        <tr class="hover:bg-slate-50 transition-colors">
                            ...
                            <td class="px-6 py-4 text-right">
                                <a href="{{ route('production.in-process-inspection.show', $inspection) }}" class="text-indigo-600 hover:text-indigo-900 font-medium text-xs uppercase tracking-wide">
