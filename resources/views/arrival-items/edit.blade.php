        ...
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
            ...
            @if ($errors->any())
                <div class="rounded-lg border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700 space-y-2">
                    <div class="font-semibold">{{ __('incoming.items.edit.error_title') }}</div>
            ...
            <div class="flex items-start justify-between gap-4">
                <div>
                    <h1 class="text-xl font-bold text-slate-900">{{ __('incoming.items.edit.title') }}</h1>
                    <p class="text-sm text-slate-600">
                        Invoice {{ $arrival->invoice_no }} • Part {{ $item->part->part_no }}
                    </p>
                </div>
                <a href="{{ route('departures.show', $arrival) }}" class="text-sm text-indigo-600 hover:text-blue-700 font-medium">
                    {{ __('incoming.items.edit.back') }}
                </a>
            </div>
                ...
                <div class="rounded-xl border border-slate-200 bg-white p-6 shadow-sm space-y-5">
                    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                        <div class="sm:col-span-2">
                            <div class="text-xs font-semibold text-slate-500">{{ __('incoming.items.edit.part') }}</div>
                        ...
                        <div class="sm:col-span-2">
                            <label for="material_group" class="text-sm font-medium text-slate-700">{{ __('incoming.items.edit.material_group') }}</label>
                            <input type="text" id="material_group" name="material_group" value="{{ old('material_group', $item->material_group) }}" class="mt-1 w-full rounded-lg border-slate-300 focus:ring-indigo-500 focus:border-indigo-500 text-sm" placeholder="{{ __('incoming.items.edit.optional') }}">
                        </div>

                        <div class="sm:col-span-2">
                            <label for="size" class="text-sm font-medium text-slate-700">{{ __('incoming.items.edit.size') }}</label>
                            <input type="text" id="size" name="size" value="{{ old('size', $item->size) }}" class="mt-1 w-full rounded-lg border-slate-300 focus:ring-indigo-500 focus:border-indigo-500 text-sm" placeholder="e.g. 0.25 x 557 x 1203">
                        </div>
                    </div>

                    <div class="rounded-xl border border-slate-200 bg-slate-50 p-4">
                        <h2 class="text-xs font-semibold tracking-wide text-slate-600 uppercase mb-3">{{ __('incoming.items.edit.detail_part') }}</h2>

                        <div class="space-y-3">
                            <div class="sm:flex sm:items-center sm:gap-4">
                                <label for="qty_goods" class="text-xs font-semibold text-slate-500 sm:w-44">{{ __('incoming.items.edit.qty_goods') }}</label>
                            ...
                            <div class="sm:flex sm:items-center sm:gap-4">
                                <label for="unit_goods" class="text-xs font-semibold text-slate-500 sm:w-44">{{ __('incoming.items.edit.unit_code') }}</label>
	                                <select id="unit_goods" name="unit_goods" class="mt-1 w-full rounded-lg border-slate-300 bg-white text-sm sm:mt-0 sm:flex-1">
	                                    ...
	                                    <option value="">{{ __('incoming.items.edit.select_unit') }}</option>
                    ...
                    <div class="rounded-xl border border-slate-200 bg-slate-50 p-4">
                        <h2 class="text-xs font-semibold tracking-wide text-slate-600 uppercase mb-3">{{ __('incoming.items.edit.detail_packaging') }}</h2>

                        <div class="space-y-3">
                            <div class="sm:flex sm:items-center sm:gap-4">
                                <label for="unit_bundle" class="text-xs font-semibold text-slate-500 sm:w-44">{{ __('incoming.items.edit.package_type') }}</label>
                                    ...
                                    <option value="PACKAGES" {{ strtoupper((string) $unitBundle) === 'PACKAGES' ? 'selected' : '' }}>PACKAGES</option>
                                    <option value="ROLL" {{ strtoupper((string) $unitBundle) === 'ROLL' ? 'selected' : '' }}>ROLL</option>
	                                </select>
	                            </div>

                            <div class="sm:flex sm:items-center sm:gap-4">
                                <label for="qty_bundle" class="text-xs font-semibold text-slate-500 sm:w-44">{{ __('incoming.items.edit.qty_package') }}</label>
                            ...
                            <div class="sm:flex sm:items-center sm:gap-4">
                                <label class="text-xs font-semibold text-slate-500 sm:w-44">{{ __('incoming.items.edit.net_weight') }}</label>
                            ...
                            <div class="sm:flex sm:items-center sm:gap-4">
                                <label class="text-xs font-semibold text-slate-500 sm:w-44">{{ __('incoming.items.edit.gross_weight') }}</label>
                                <div class="mt-1 flex w-full items-center gap-2 sm:mt-0 sm:flex-1">
                            ...
                            <div class="sm:flex sm:items-start sm:gap-4">
                                <label for="total_amount" class="text-xs font-semibold text-slate-500 sm:w-44 sm:pt-2">{{ __('incoming.items.edit.total_price') }}</label>
                                <div class="mt-1 w-full sm:mt-0 sm:flex-1">
                                    <input type="text" inputmode="decimal" id="total_amount" name="total_amount" value="{{ old('total_amount', $item->total_price) }}" class="w-full rounded-lg border-blue-300 bg-white text-sm focus:border-indigo-500 focus:ring-indigo-500" required>
                                    <div class="mt-1 text-[11px] text-slate-500">{{ __('incoming.items.edit.price_hint') }}</div>
                                </div>
                            </div>

                            <div class="sm:flex sm:items-center sm:gap-4">
                                <label class="text-xs font-semibold text-slate-500 sm:w-44">{{ __('incoming.items.edit.price_auto') }}</label>
                    ...
                    <div>
                        <label class="inline-flex items-center gap-3 rounded-lg border border-slate-200 bg-slate-50 px-3 py-2">
                            ...
                            <input type="checkbox" name="is_foc" value="1" @checked(old('is_foc', $item->is_foc)) class="rounded border-slate-300 text-indigo-600 focus:ring-indigo-500">
                            <span class="text-sm font-medium text-slate-700">{{ __('incoming.items.edit.foc') }}</span>
                        </label>
                    </div>

                    <div>
                        <label for="notes" class="text-sm font-medium text-slate-700">{{ __('incoming.items.edit.notes') }}</label>
                        <textarea id="notes" name="notes" rows="3" class="mt-1 w-full rounded-lg border-slate-300 focus:ring-indigo-500 focus:border-indigo-500 text-sm">{{ old('notes', $item->notes) }}</textarea>
                    </div>
                    ...
                    <div class="flex items-center justify-end gap-3">
                        <a href="{{ route('departures.show', $arrival) }}" class="px-4 py-2 rounded-lg border border-slate-300 text-sm font-medium text-slate-700 hover:bg-slate-50">
                            {{ __('incoming.items.edit.cancel') }}
                        </a>
                        <button type="submit" class="px-5 py-2.5 rounded-lg bg-indigo-600 text-white text-sm font-semibold hover:bg-indigo-700 transition">
                            {{ __('incoming.items.edit.save') }}
                        </button>
                    </div>
                </div>
            </form>
