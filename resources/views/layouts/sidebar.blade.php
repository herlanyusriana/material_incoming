@php
    ...
    $qualityRoutesActive = request()->routeIs('production.qc-inspection.*')
        || request()->routeIs('production.in-process-inspection.*')
        || request()->routeIs('production.final-inspection.*')
        || request()->routeIs('production.kanban-update.*');
    $canAccessProductionSection = auth()->user()?->can('view_production')
        || auth()->user()?->can('manage_qc_inspection')
        || auth()->user()?->can('manage_in_process_inspection')
        || auth()->user()?->can('manage_final_inspection')
        || auth()->user()?->can('manage_kanban_update');
            ...
            @can('manage_planning')
                <div>
                    ...
                    <div class="space-y-1">
                        ...
                        <a href="{{ route('machines.index') }}" @class([$navLinkBase, $navActive => request()->routeIs('machines.*'), $navInactive => !request()->routeIs('machines.*')])
                            @click="mobileSidebarOpen = false">
                            <svg xmlns="http://www.w3.org/2000/svg" class="{{ $navIconBase }}" fill="none"
                                viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                    ...
                    <div class="space-y-1">
                        <details class="group" {{ request()->routeIs('planning.*') ? 'open' : '' }}>
                            ...
                            <div class="relative mt-2 ml-4 pl-4 space-y-1">
                                ...
                                <a href="{{ route('planning.mrp.integration-dashboard') }}" @class([$subLinkBase, $subActive => request()->routeIs('planning.mrp.integration-dashboard'), $subInactive => !request()->routeIs('planning.mrp.integration-dashboard')])
                                    @click="mobileSidebarOpen = false">
                            ...
                            <div class="relative mt-2 ml-4 pl-4 space-y-1">
                                ...
                                @can('manage_qc_inspection')
                                    <a href="{{ route('production.qc-inspection.index') }}" @class([$subLinkBase, $subActive => request()->routeIs('production.qc-inspection.*'), $subInactive => !request()->routeIs('production.qc-inspection.*')]) @click="mobileSidebarOpen = false">
                                        <span @class([$subDotBase, 'bg-amber-600' => request()->routeIs('production.qc-inspection.*'), 'bg-slate-300 group-hover:bg-amber-400' => !request()->routeIs('production.qc-inspection.*')])></span>
                                        <span class="flex-1">QC Inspection</span>
                                    </a>
                                @endcan
                                @can('manage_in_process_inspection')
                                    <a href="{{ route('production.in-process-inspection.index') }}" @class([$subLinkBase, $subActive => request()->routeIs('production.in-process-inspection.*'), $subInactive => !request()->routeIs('production.in-process-inspection.*')]) @click="mobileSidebarOpen = false">
                                        <span @class([$subDotBase, 'bg-sky-600' => request()->routeIs('production.in-process-inspection.*'), 'bg-slate-300 group-hover:bg-sky-400' => !request()->routeIs('production.in-process-inspection.*')])></span>
                        ...
                        <div class="mt-2">
                            <div class="relative ml-4 pl-4 space-y-1">
                                ...
                                <a href="{{ route('outgoing.osp.index') }}" @class([$subLinkBase, $subActive => request()->routeIs('outgoing.osp.*'), $subInactive => !request()->routeIs('outgoing.osp.*')]) @click="mobileSidebarOpen = false">
                                    ...
                                    <span class="flex-1">OSP Outgoing Docs</span>
                                </a>
        ...
        @can('manage_planning')
            <div>
                ...
                <div class="space-y-1">
                    ...
                    <a href="{{ route('parts.index') }}" title="Parts Master" @class([$navLinkBase, $navActive => $partsActive, $navInactive => !$partsActive]) :class="sidebarCollapsed ? 'justify-center' : 'gap-3'">
                        <svg xmlns="http://www.w3.org/2000/svg" class="{{ $navIconBase }}" fill="none" viewBox="0 0 24 24"
                            stroke="currentColor" stroke-width="2">
                            <path stroke-linecap="round" stroke-linejoin="round"
                                d="M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16Z" />
                    ...
                    <a href="{{ route('machines.index') }}" title="Machines" @class([$navLinkBase, $navActive => request()->routeIs('machines.*'), $navInactive => !request()->routeIs('machines.*')])
                        :class="sidebarCollapsed ? 'justify-center' : 'gap-3'">
                        <svg xmlns="http://www.w3.org/2000/svg" class="{{ $navIconBase }}" fill="none" viewBox="0 0 24 24"
                            stroke="currentColor" stroke-width="2">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z" />
                        ...
                        <span x-show="!sidebarCollapsed" x-cloak>Machines</span>
                    </a>
                </div>
            </div>
        @endcan
        ...
        @can('manage_inventory')
            <div>
                ...
                <div class="space-y-1" x-show="!sidebarCollapsed" x-cloak>
                    ...
                    <a href="{{ route('warehouse.batch-transfers.index') }}" @class([$navLinkBase, $navActive => request()->routeIs('warehouse.batch-transfers.*'), $navInactive => !request()->routeIs('warehouse.batch-transfers.*')])>
                        <svg xmlns="http://www.w3.org/2000/svg" class="{{ $navIconBase }}" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M7 7h10M7 12h10M7 17h10" />
                            <path stroke-linecap="round" stroke-linejoin="round" d="M5 5h14v14H5z" />
                        </svg>
