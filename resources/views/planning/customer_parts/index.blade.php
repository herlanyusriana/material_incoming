    ...
    <div class="py-3" x-data="planningCustomerParts()" x-init="init()">
                    ...
                    @forelse ($customerParts as $cp)
                        <div class="border border-slate-200 rounded-2xl p-4">
                            <div class="flex flex-wrap items-start justify-between gap-3">
                                <div>
                                    <div class="text-xs text-slate-500">{{ $cp->customer->code ?? '-' }} •
                                        {{ $cp->customer->name ?? '-' }}</div>
                                ...
                                <div class="flex items-center gap-2">
                                    <span
                                        class="inline-flex items-center px-2 py-1 rounded-full text-xs font-semibold {{ $cp->status === 'active' ? 'bg-emerald-100 text-emerald-800' : 'bg-slate-100 text-slate-700' }}">
                                            ...
                                            @forelse ($cp->components as $comp)
                                                <tr class="hover:bg-slate-50 transition-colors">
                                                    <td class="px-3 py-2 font-semibold text-slate-700">
                                                        {{ $comp->part->part_no ?? '-' }}</td>
                                                    <td class="px-3 py-2 text-slate-600 text-xs">
                                                        {{ $comp->part->part_name ?? '-' }}</td>
                                                    <td
                                                        class="px-3 py-2 text-right font-mono text-xs text-indigo-600 font-bold whitespace-nowrap">
                                                        {{ number_format((float) $comp->qty_per_unit, 3) }}</td>
                                                </tr>
                                            @empty
                                                <tr>
                ...
                <div class="flex items-center justify-between px-5 py-4 border-b border-slate-200">
                    <div>
                        <div class="text-xs text-slate-500 uppercase font-bold tracking-wider"
                            x-text="compForm.customerCode"></div>
        ...
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-sm px-4"
            ...
            <div class="w-full max-w-lg bg-white rounded-2xl shadow-xl border border-slate-200">
                ...
                <form action="{{ route('planning.customer-parts.import') }}" method="POST" enctype="multipart/form-data"
                    class="p-5 space-y-4">
                    @csrf
                    <div>
            ...
            function planningCustomerParts() {
                return {
                    ...
                    openCreate() {
                        ...
                        this.form = { id: null, customer_id: '', customer_part_no: '', customer_part_name: '', line: '', case_name: '', status: 'active' };
                        this.modalOpen = true;
                    },
