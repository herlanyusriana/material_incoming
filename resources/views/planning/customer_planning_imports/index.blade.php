        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="bg-white shadow-lg border border-slate-200 rounded-2xl p-6 space-y-4">
                <div class="flex flex-wrap items-start justify-between gap-3">
                    <div class="text-sm text-slate-600">Upload customer planning Excel. Period format: YYYY-MM or
                        YYYY-WW.</div>
                    <div class="flex items-center gap-2">
                        <a href="{{ route('planning.planning-imports.template') }}"
                            class="px-4 py-2 rounded-xl bg-slate-900 text-white font-semibold">Template Weekly</a>
                        <a href="{{ route('planning.planning-imports.template-monthly') }}"
                            class="px-4 py-2 rounded-xl bg-white border border-slate-200 hover:bg-slate-50 font-semibold">Template
                            Monthly</a>
                        @if ($importId)
                            <a href="{{ route('planning.planning-imports.export', $importId) }}"
                                class="px-4 py-2 rounded-xl bg-white border border-slate-200 hover:bg-slate-50 font-semibold">Export
                                Rows</a>
                        @endif
                    </div>
                </div>
                <div class="text-xs text-slate-500">
                    Monthly tips: pakai header bulan format <span class="font-mono">YYYY-MM</span> (recommended).
                    WeekMap ratio boleh kosong, nanti dibagi rata otomatis.
                </div>

                <form action="{{ route('planning.planning-imports.store') }}" method="POST"
                        ...
                        <tbody class="divide-y divide-slate-100">
                            @forelse ($imports as $imp)
                                <tr class="hover:bg-slate-50">
                                    ...
                                    <td class="px-4 py-3 text-right font-mono text-xs text-emerald-600">
                                        {{ $imp->accepted_rows }}</td>
                                    <td class="px-4 py-3 text-right font-mono text-xs text-red-600">
                                        {{ $imp->rejected_rows }}</td>
                                    <td class="px-4 py-3">{{ strtoupper($imp->status) }}</td>
                                    <td class="px-4 py-3 text-right whitespace-nowrap">
                                        <a class="inline-flex items-center px-3 py-1.5 rounded-lg border border-slate-200 hover:bg-slate-50 font-semibold text-xs"
                                            href="{{ route('planning.planning-imports.export', $imp) }}">Export</a>
                                        <a class="inline-flex items-center px-3 py-1.5 rounded-lg bg-indigo-600 hover:bg-indigo-700 text-white font-semibold text-xs"
                                            href="{{ route('planning.planning-imports.index', ['import_id' => $imp->id]) }}">View
                                            Rows</a>
                                    </td>
                                </tr>
                            @empty
                            ...
                            <tr class="hover:bg-slate-50">
                                ...
                                <td class="px-4 py-3">
                                    @if($row->row_status === 'accepted')
                                        <span
                                            class="inline-flex items-center px-2 py-1 rounded-md text-xs font-semibold bg-emerald-100 text-emerald-800">
                                    @elseif($row->row_status === 'unknown_mapping')
                                            <span
                                                class="inline-flex items-center px-2 py-1 rounded-md text-xs font-semibold bg-amber-100 text-amber-800">
                                        @else
                                                <span
                                                    class="inline-flex items-center px-2 py-1 rounded-md text-xs font-semibold bg-red-100 text-red-800">
                                            @endif
