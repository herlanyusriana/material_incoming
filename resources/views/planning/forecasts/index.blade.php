            ...
            <div class="bg-white shadow-lg border border-slate-200 rounded-2xl p-6 space-y-4">
                <div class="flex flex-wrap items-end justify-between gap-3">
                    ...
                    <form method="POST" action="{{ route('planning.forecasts.clear') }}"
                        ...
                        @method('DELETE')
                        <button type="submit"
                            class="px-4 py-2 rounded-xl font-semibold border bg-red-600 border-red-600 text-white hover:bg-red-700">
                            🗑️ Clear All
                        </button>
                    </form>
                </div>
                        ...
                        <tbody class="divide-y divide-slate-100">
                            @forelse ($forecasts as $f)
                                <tr class="hover:bg-slate-50">
                                    ...
                                    <td class="px-4 py-3 text-right font-mono text-xs">{{ formatNumber($f->planning_qty) }}
                                    </td>
                                    ...
                                    <td class="px-4 py-3 text-right font-mono text-xs font-semibold">
                                        {{ formatNumber($f->qty) }}</td>
                                    <td class="px-4 py-3 text-xs uppercase tracking-wide text-slate-600">{{ $f->source }}
                                    </td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="5" class="px-4 py-8 text-center text-slate-500">No forecasts. Click
                                        Generate.</td>
                                </tr>
                            @endforelse
                        </tbody>
