<x-app-layout>
    <x-slot name="header">
        {{ __('planning.forecasts.index.header') }}
    </x-slot>

    <x-page-header
        title="{{ __('planning.forecasts.index.title') }}"
        subtitle="{{ __('planning.forecasts.index.subtitle') }}"
        :breadcrumbs="[
            ['label' => __('planning.forecasts.index.crumb_planning'), 'url' => null],
            ['label' => __('planning.forecasts.index.crumb_forecast')]
        ]"
    >
        <x-slot name="actions">
            <form method="POST" action="{{ route('planning.forecasts.preview-plan') }}"
                ...
                <label for="plan-file-input"
                    ...
                    <span>{{ __('planning.forecasts.index.upload') }}</span>
                </label>
            </form>
            ...
            <a href="{{ route('planning.forecasts.history') }}"
                ...
                <span>{{ __('planning.forecasts.index.history') }}</span>
            </a>

            <form method="POST" action="{{ route('planning.forecasts.clear') }}"
                onsubmit='return confirm(@js(__("planning.forecasts.index.confirm_clear")));'
                class="inline">
                @csrf
                @method('DELETE')
                <button type="submit"
                    ...
                    <span>{{ __('planning.forecasts.index.clear') }}</span>
                </button>
            </form>
        </x-slot>
    </x-page-header>
    ...
    <div class="space-y-6">
        ...
        @if ($errors->any())
            <div class="rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">
                <div class="font-bold">{{ __('planning.forecasts.index.validation') }}</div>
        ...
        <div class="grid gap-4 sm:grid-cols-4">
            <div class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                <div class="text-xs uppercase tracking-wider text-slate-500 font-semibold">{{ __('planning.forecasts.index.kpi_total') }}</div>
                <div class="mt-2 text-3xl font-black text-slate-900">{{ number_format((int) ($kpi->total_rows ?? 0)) }}</div>
                <div class="text-xs text-slate-400 mt-1">{{ __('planning.forecasts.index.kpi_total_hint') }}</div>
            </div>
            <div class="rounded-2xl border border-indigo-200 bg-indigo-50 p-5 shadow-sm">
                <div class="text-xs uppercase tracking-wider text-indigo-700 font-semibold">{{ __('planning.forecasts.index.kpi_planning') }}</div>
                <div class="mt-2 text-3xl font-black text-indigo-900">{{ formatNumber((float) ($kpi->total_planning ?? 0)) }}</div>
                <div class="text-xs text-indigo-600/70 mt-1">{{ __('planning.forecasts.index.kpi_planning_hint') }}</div>
            </div>
            <div class="rounded-2xl border border-amber-200 bg-amber-50 p-5 shadow-sm">
                <div class="text-xs uppercase tracking-wider text-amber-700 font-semibold">{{ __('planning.forecasts.index.kpi_po') }}</div>
                <div class="mt-2 text-3xl font-black text-amber-900">{{ formatNumber((float) ($kpi->total_po ?? 0)) }}</div>
                <div class="text-xs text-amber-600/70 mt-1">{{ __('planning.forecasts.index.kpi_po_hint') }}</div>
            </div>
            <div class="rounded-2xl border border-emerald-200 bg-emerald-50 p-5 shadow-sm">
                <div class="text-xs uppercase tracking-wider text-emerald-700 font-semibold">{{ __('planning.forecasts.index.kpi_forecast') }}</div>
                <div class="mt-2 text-3xl font-black text-emerald-900">{{ formatNumber((float) ($kpi->total_qty ?? 0)) }}</div>
                <div class="text-xs text-emerald-600/70 mt-1">{{ __('planning.forecasts.index.kpi_forecast_hint') }}</div>
            </div>
        </div>
                ...
                <div class="flex gap-2">
                    ...
                    @if ($period !== '' || $partId !== '' || $family !== '')
                        <a href="{{ route('planning.forecasts.index') }}"
                            class="px-4 py-2 rounded-xl border bg-white border-slate-200 text-slate-700 hover:bg-slate-50">{{ __('planning.forecasts.index.filter_reset') }}</a>
                    @endif
                </div>
            </form>
        </div>
        ...
        <div class="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm">
                ...
                <table class="min-w-full text-sm divide-y divide-slate-200">
                    <thead class="bg-slate-50">
                        <tr class="text-slate-600 text-xs uppercase tracking-wider">
                            <th class="px-4 py-3 text-left font-semibold">{{ __('planning.forecasts.index.th_part') }}</th>
                            <th class="px-4 py-3 text-left font-semibold">{{ __('planning.forecasts.index.th_family') }}</th>
                            ...
                            @endforeach
                            <th class="px-4 py-3 text-right font-semibold">{{ __('planning.forecasts.index.th_total') }}</th>
                        </tr>
                    </thead>
                            ...
                            <tr>
                                <td colspan="{{ 2 + $months->count() + 1 }}" class="px-4 py-12 text-center text-slate-400">
                                    <div class="flex flex-col items-center gap-2">
                                        ...
                                        <span class="text-sm font-medium">{{ __('planning.forecasts.index.empty_title') }}</span>
                                        <span class="text-xs">{{ __('planning.forecasts.index.empty_hint') }}</span>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
            ...
            <div class="border-t border-slate-200 px-4 py-3 bg-slate-50/50 flex flex-wrap items-center justify-between gap-2">
                <div class="text-xs text-slate-500">
                    {{ __('planning.forecasts.index.showing') }} <span class="font-semibold text-slate-700">{{ $forecasts->firstItem() ?? 0 }}</span> -
                    <span class="font-semibold text-slate-700">{{ $forecasts->lastItem() ?? 0 }}</span>
                    {{ __('planning.forecasts.index.of') }} <span class="font-semibold text-slate-700">{{ $forecasts->total() }}</span> {{ __('planning.forecasts.index.rows') }}
                </div>
