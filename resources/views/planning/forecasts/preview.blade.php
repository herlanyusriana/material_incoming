        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
            <div class="flex justify-between items-center">
                <div>
                    ...
                    <p class="text-sm text-slate-600 mt-1">Choose individual PO Numbers and Planning Files to include in
                        forecast</p>
                </div>
                ...
                <div class="bg-white border rounded-xl shadow-sm overflow-hidden">
                    <div class="px-6 py-4 bg-blue-50 border-b flex justify-between items-center">
                        ...
                        <label class="flex items-center gap-2 cursor-pointer">
                            <input type="checkbox" x-model="selectAllPos"
                                @change="selectedPos = selectAllPos ? {{ $customerPos->pluck('id')->toJson() }} : []"
                            ...
                            <table class="w-full text-sm">
                                <thead class="text-xs text-slate-500 uppercase bg-slate-50 border-b">
                                    <tr>
                                        ...
                                        <th class="px-6 py-3 text-center font-semibold">PERIOD</th>
                                ...
                                <tbody class="divide-y divide-slate-100">
                                    @foreach($customerPos as $po)
                                        <tr class="hover:bg-blue-50 transition-colors"
                                            :class="selectedPos.includes({{ $po->id }}) ? 'bg-blue-50' : ''">
                                            <td class="px-6 py-4">
                                                <input type="checkbox" name="selected_pos[]" value="{{ $po->id }}"
                                                    x-model.number="selectedPos" class="rounded border-slate-300 text-blue-600">
                                            </td>
                                            <td class="px-6 py-4">
                                            ...
                                            <td class="px-6 py-4 text-center">
                                                <span
                                                    class="px-2 py-0.5 bg-slate-100 rounded-md font-mono text-xs font-semibold text-slate-600">
                                                    {{ $po->period }}
                                                </span>
                                            </td>
                                            <td class="px-6 py-4 text-right font-mono font-bold text-blue-700">
                                                {{ formatNumber($po->qty) }}
                                            </td>
                ...
                <div class="bg-white border rounded-xl shadow-sm overflow-hidden mt-6">
                    <div class="px-6 py-4 bg-green-50 border-b flex justify-between items-center">
                        <div>
                            ...
                            <p class="text-xs text-green-700 mt-1">{{ $planningImports->count() }} accepted files
                                available</p>
                        </div>
                        <label class="flex items-center gap-2 cursor-pointer">
                            <input type="checkbox" x-model="selectAllImports"
                                @change="selectedImports = selectAllImports ? {{ $planningImports->pluck('id')->toJson() }} : []"
                                ...
                                <tbody class="divide-y divide-slate-100">
                                    @foreach($planningImports as $import)
                                        <tr class="hover:bg-green-50 transition-colors"
                                            :class="selectedImports.includes({{ $import->id }}) ? 'bg-green-50' : ''">
                                            <td class="px-6 py-4">
                                                <input type="checkbox" name="selected_imports[]" value="{{ $import->id }}"
                                            ...
                                            <td class="px-6 py-4">
                                                ...
                                                <div class="text-[10px] text-slate-400 font-mono italic">Import ID:
                                                    #{{ $import->id }}</div>
                                            </td>
                                            <td class="px-6 py-4">
                                                <span
                                                    class="font-semibold text-slate-700">{{ $import->customer->name ?? '-' }}</span>
                                            </td>
                                            <td class="px-6 py-4 text-center">
                                                <span
                ...
                <div class="bg-white border rounded-xl shadow-sm p-6 mt-6 sticky bottom-6 z-10">
                    <div class="flex justify-between items-center">
                        <div class="text-sm text-slate-600">
                            ...
                            <span class="font-semibold text-green-700" x-text="selectedImports.length"></span> Planning
                            file(s) selected
                        </div>
                        <div class="flex gap-3">
                            <a href="{{ route('planning.forecasts.index') }}"
                                class="px-6 py-2 bg-slate-200 text-slate-700 rounded-lg hover:bg-slate-300 font-semibold">
                                Cancel
                            </a>
                            <button type="submit"
