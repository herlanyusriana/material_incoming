...
<template x-if="mode === 'edit' && form.classification === 'RM'">
    <div class="rounded-xl border border-orange-200 bg-orange-50/50 overflow-hidden">
        <button type="button" class="w-full flex items-center justify-between px-3 py-2 text-sm font-semibold text-orange-800 hover:bg-orange-100/60"
            ...
            <span>{{ __('planning.gci_parts.substitutes.sub_title') }}</span>
        ...
        <div x-show="subsOpen" x-cloak class="px-3 pb-3 space-y-3">
                ...
                <table class="w-full text-xs divide-y divide-slate-100 bg-white rounded-lg border border-slate-200">
                    <thead>
                        <tr class="text-slate-400 uppercase tracking-wider text-[10px]">
                            <th class="text-left py-2 px-2">{{ __('planning.gci_parts.substitutes.sub_th_fg') }}</th>
                            <th class="text-left py-2 px-2">{{ __('planning.gci_parts.substitutes.sub_th_sub') }}</th>
                            <th class="text-right py-2 px-2">{{ __('planning.gci_parts.substitutes.sub_th_ratio') }}</th>
                            <th class="text-right py-2 px-2">{{ __('planning.gci_parts.substitutes.sub_th_prio') }}</th>
                        ...
                        <template x-for="s in form.substitutes_for" :key="s.id">
                            <tr class="hover:bg-slate-50">
                                ...
                                <td class="py-1.5 px-2 text-right whitespace-nowrap">
                                    <button type="button" class="font-semibold text-indigo-600 hover:text-indigo-800" @click="editSub(s)">{{ __('planning.gci_parts.index.edit') }}</button>
                                    <button type="button" class="ml-2 font-semibold text-red-600 hover:text-red-800" @click="deleteSub(s)">{{ __('planning.gci_parts.index.delete') }}</button>
                                </td>
                            </tr>
                        </template>
                    </tbody>
            ...
            <template x-if="!(form.substitutes_for || []).length">
                <div class="text-xs text-slate-400 italic py-1">{{ __('planning.gci_parts.substitutes.sub_empty') }}</div>
            </template>
            ...
            <form :action="subFormAction" method="POST" class="space-y-2 rounded-lg border border-orange-200 bg-white p-3">
                ...
                <div class="grid grid-cols-2 gap-2">
                    <div>
                        <label class="text-xs font-semibold text-slate-600">{{ __('planning.gci_parts.substitutes.sub_fg') }} <span class="text-red-600">*</span></label>
                        <select name="fg_part_id" required class="mt-0.5 w-full rounded-lg border-slate-200 text-sm" x-model="subForm.fg_part_id" :disabled="!!subEditId">
                            <option value="">{{ __('planning.gci_parts.substitutes.sub_select_fg') }}</option>
                    ...
                    <div>
                        <label class="text-xs font-semibold text-slate-600">{{ __('planning.gci_parts.substitutes.sub_rm') }} <span class="text-red-600">*</span></label>
                        <select name="substitute_part_id" required class="mt-0.5 w-full rounded-lg border-slate-200 text-sm" x-model="subForm.substitute_part_id">
                            <option value="">{{ __('planning.gci_parts.substitutes.sub_select_rm') }}</option>
                ...
                <div class="grid grid-cols-4 gap-2">
                    <input type="number" name="ratio" step="0.001" min="0.001" placeholder="{{ __('planning.gci_parts.substitutes.sub_ratio_ph') }}" class="w-full rounded-lg border-slate-200 text-sm" x-model="subForm.ratio">
                    <input type="number" name="priority" min="1" placeholder="{{ __('planning.gci_parts.substitutes.sub_priority_ph') }}" class="w-full rounded-lg border-slate-200 text-sm" x-model="subForm.priority">
                    ...
                    <input type="text" name="notes" maxlength="255" placeholder="{{ __('planning.gci_parts.substitutes.sub_notes_ph') }}" class="w-full rounded-lg border-slate-200 text-sm" x-model="subForm.notes">
                </div>
                <div class="flex justify-end gap-2">
                    <button type="button" x-show="subEditId" @click="cancelSubEdit()"
                        class="px-3 py-1.5 rounded-lg border border-slate-200 hover:bg-slate-50 text-xs font-semibold">{{ __('planning.gci_parts.index.cancel') }}</button>
                    <button type="submit" class="px-3 py-1.5 rounded-lg bg-orange-600 hover:bg-orange-700 text-white text-xs font-semibold"
                        x-text="subEditId ? '{{ __('planning.gci_parts.substitutes.sub_update') }}' : '{{ __('planning.gci_parts.substitutes.sub_add') }}'"></button>
                </div>
            </form>
        </div>
    </div>
