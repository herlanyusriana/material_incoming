            ...
            <div class="bg-white shadow-lg border border-slate-200 rounded-2xl p-6 space-y-4"
                ...
                <div class="flex flex-wrap items-end justify-between gap-3">
                    ...
                    <form method="POST" action="{{ route('planning.mrp.generate-range') }}"
                        ...
                        <span class="text-xs text-slate-500 italic mr-2">Run MRP for {{ $weeksCount }} weeks
                            ({{ $startWeek }} ...)</span>
                    ...
                    <form method="POST" action="{{ route('planning.mrp.clear') }}" id="clear-form"
                        onsubmit="return confirm('Clear ALL MRP data?');" class="hidden">
                        @csrf
                        @method('DELETE')
                    </form>
                </div>
                ...
                @if(!empty($mrpDataMake) && empty($mrpDataBuy))
                    <div class="rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900">
                        Demand/incoming komponen BUY belum muncul karena MRP Purchase Plan masih kosong. Biasanya ini karena
                        BOM belum ter-relasi ke master part: `bom_items.component_part_id` masih kosong atau
                        `component_part_no` belum ada di `gci_parts.part_no`.
                    </div>
                @endif
                ...
                @else
                    <div class="space-y-6" x-show="(tab === 'buy' || tab === 'all') && viewMode === 'daily'" x-cloak>
                        @include('planning.mrp.partials.table', ['mrpRows' => $mrpDataBuy ?? [], 'modeLabel' => 'Purchase Planning (BUY) • Daily', 'showPoAction' => true, 'showIncoming' => true, 'month' => $month])
                    </div>
                    ...
                    <div class="space-y-6" x-show="(tab === 'make' || tab === 'all') && viewMode === 'daily'" x-cloak>
                        @include('planning.mrp.partials.table', ['mrpRows' => $mrpDataMake ?? [], 'modeLabel' => 'Production Planning (MAKE) • Daily', 'showPoAction' => false, 'showIncoming' => false, 'month' => $month])
                    </div>
