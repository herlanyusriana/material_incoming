<div class="space-y-6">
    ...
    <div class="bg-slate-50 border border-slate-200 rounded-lg p-5">
        <div class="flex justify-between items-start mb-4">
            ...
            <div class="text-right">
                <span
                    class="block text-xs uppercase tracking-wider text-slate-400 font-semibold">{{ preg_match('/^\d{4}-\d{2}$/', $period) ? 'Month' : 'Week' }}</span>
                <span class="block text-lg font-mono font-bold text-indigo-600">
                    {{ preg_match('/^\d{4}-\d{2}$/', $period) ? \Carbon\Carbon::parse($period . '-01')->format('F Y') : $period }}
                </span>
            </div>
        </div>
        ...
        <div class="grid grid-cols-2 gap-4 text-sm">
            ...
            <div class="bg-white p-3 rounded border border-slate-100">
                ...
                <span
                    class="inline-flex items-center px-2 py-0.5 rounded text-xs font-bold mt-1 
    ...
    <div class="bg-white border rounded-lg shadow-sm p-6 relative">
        ...
        <form action="{{ route('planning.mps.upsert') }}" method="POST">
            ...
            @if($mps->status !== 'approved')
                <div class="flex gap-3">
                    ...
                    @if($mps->exists)
                        <button type="submit" name="action" value="approve" form="approve-single-form"
                            onclick="return confirm('Approve this plan? It will be locked.')"
                            class="flex-1 flex justify-center py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-emerald-600 hover:bg-emerald-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-emerald-500">
                            Approve
                        </button>
                    @endif
                </div>
        ...
        @if($mps->exists && $mps->status !== 'approved')
            <form id="approve-single-form" action="{{ route('planning.mps.approve') }}" method="POST" class="hidden">
                @csrf
                <input type="hidden" name="period" value="{{ $period }}">
                <input type="hidden" name="mps_ids[]" value="{{ $mps->id }}">
            </form>
        @endif
    </div>
            ...
            <div class="max-h-60 overflow-y-auto">
                <table class="min-w-full divide-y divide-slate-100">
                    <thead class="bg-white sticky top-0">
                        <tr>
                            <th class="px-5 py-2 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">
                                Component</th>
                            <th class="px-5 py-2 text-right text-xs font-medium text-slate-500 uppercase tracking-wider">
                                Usage</th>
                            <th class="px-5 py-2 text-right text-xs font-medium text-slate-500 uppercase tracking-wider">
                                Est. Req</th>
                        </tr>
                    ...
                    <tbody class="divide-y divide-slate-50">
                        @foreach($bom->items as $item)
                            <tr>
                                ...
                                <td
                                    class="px-5 py-2 whitespace-nowrap text-sm text-right font-mono text-indigo-600 font-semibold">
