                            ...
                            @forelse ($boms as $bom)
                                ...
                                @forelse($items as $idx => $item)
                                    ...
                                    <tr class="child-row group" x-show="expanded[{{ $bomId }}]" x-cloak>
                                        ...
                                        <td
                                            class="px-2 py-1.5 whitespace-nowrap text-[10px] font-bold text-slate-500 uppercase">
                                            {{ $item->consumptionUom?->code ?? ($item->consumption_uom ?? '') }}</td>
                                        <td
            ...
            <div class="w-full max-w-lg bg-white rounded-2xl shadow-xl border border-slate-200">
                ...
                <form action="{{ route('planning.boms.import') }}" method="POST" enctype="multipart/form-data"
                    ...
                    <div class="text-sm text-slate-700 space-y-1">
                        <div>Gunakan format kolom yang sama seperti hasil <span class="font-semibold">Export BOM</span>.
                        </div>
                    ...
                    <form action="{{ route('planning.boms.substitutes.import') }}" method="POST"
                        ...
                        <div class="text-sm text-slate-700 space-y-1">
                            ...
                            <div class="font-mono text-xs bg-slate-100 p-2 rounded">fg_part_no, fg_part_name,
                                component_part_no, component_part_name, substitute_part_no, substitute_part_name, ratio,
                                priority, status</div>
                            <div class="text-xs text-slate-500">Pastikan FG Part No dan Component Part No sesuai untuk
                    ...
                    <form action="{{ route('planning.boms.substitutes.import-mapping') }}" method="POST"
                        ...
                        <div class="text-sm text-slate-700 space-y-1">
                            ...
                            <div class="text-xs text-slate-500">Akan diterapkan ke semua BOM line yang memakai
                                `component_part_no`. Disarankan matikan auto-create agar tidak membuat part baru tanpa
                                kontrol.</div>
                        </div>
                ...
                <form :action="lineForm.action" method="POST" class="px-5 py-4 space-y-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
                        <div>
                            ...
                            <input type="number" min="1" name="line_no" class="mt-1 w-full rounded-xl border-slate-200"
                                x-model="lineForm.line_no">
                        </div>
                        <div>
                            ...
                            <div class="flex gap-2">
                                <template x-if="!lineForm.wip_part_id">
                                    <input type="text" name="wip_part_no"
                                        class="mt-1 w-full rounded-xl border-slate-200" x-model="lineForm.wip_part_no"
                                        placeholder="Part No String">
                                </template>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-4 gap-3">
                                        ...
                                        <optgroup label="MAKE (FG/WIP)">
                                            @foreach (($makeParts ?? []) as $c)
                                                <option value="{{ optional($c)->id }}">{{ optional($c)->part_no }} —
                                                    ...
                                                    ({{ strtoupper(optional($c)->classification ?? '') }})</option>
                                            @endforeach
                                        </optgroup>
                                    </template>
                                </select>
                        ...
                        <div>
                            ...
                            <input type="number" step="any" min="0" name="usage_qty"
                                class="mt-1 w-full rounded-xl border-slate-200" required x-model="lineForm.usage_qty">
                        </div>
                    </div>
                ...
                <form :action="changeFgForm.action" method="POST" class="px-5 py-4 space-y-4">
                    ...
                    <div>
                        ...
                        <div class="mt-2 text-xs text-slate-500">
                            Note: FG hanya bisa dipindah ke part classification <span class="font-semibold">FG</span>
        ...
        <div class="fixed inset-0 z-50" x-show="substituteOpen" x-cloak>
            ...
            <div
                ...
                <div class="p-5 space-y-4">
                    ...
                    <div class="space-y-2">
                        <div class="text-xs font-semibold text-slate-700 uppercase tracking-wider">Existing Substitutes
                        </div>
