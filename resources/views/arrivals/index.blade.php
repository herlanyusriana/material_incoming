            ...
            <div class="bg-white shadow-lg border border-slate-200 rounded-2xl p-6 space-y-6">
                <div class="flex items-center justify-between pb-4 border-b border-slate-200">
                    ...
                    <a href="{{ route('departures.create') }}"
                        class="inline-flex items-center gap-2 px-5 py-2.5 bg-blue-600 hover:bg-blue-700 text-white font-semibold rounded-lg transition-colors shadow-sm">
                                    ...
                                    <td class="px-4 py-4 text-slate-700">
                                        <div class="font-medium flex items-center gap-2">
                                            ...
                                            @if ($isReceiveComplete)
                                                <span
                                                    class="inline-flex items-center px-2 py-0.5 rounded-full text-[11px] font-semibold bg-emerald-100 text-emerald-700">
                                                    Complete
                                                </span>
                                            @endif
                                            @if ($arrival->purchaseOrder)
                                                <span
                                                    ...
                                                    {{ $arrival->purchaseOrder->po_number }}
                                                </span>
                                            @endif
                                        </div>
                                        <div class="text-xs text-slate-500">{{ $arrival->invoice_date?->format('d M Y') }}
                                        </div>
                                    </td>
                                    <td class="px-4 py-4 text-slate-700">{{ $arrival->vendor->vendor_name ?? '-' }}</td>
                                    <td class="px-4 py-4 text-slate-700 text-xs font-mono">
                                        {{ $arrival->ETD?->format('Y-m-d') ?? '-' }}</td>
                                    <td class="px-4 py-4 text-slate-700 text-xs font-mono">
                                        {{ $arrival->ETA?->format('Y-m-d') ?? '-' }}</td>
                                    <td class="px-4 py-4 text-slate-700 text-xs font-mono">
                                        {{ $arrival->ETA_GCI?->format('Y-m-d') ?? '-' }}</td>
                                    <td class="px-4 py-4 text-slate-700">
                                        <div class="font-semibold">{{ $totalItems }} item{{ $totalItems != 1 ? 's' : '' }}
                                        </div>
                                        <div class="text-xs text-slate-500">{{ number_format($totalQtyExpected) }} pcs total
                                        </div>
                                    </td>
                                    <td class="px-4 py-4">
                                        <div class="font-semibold text-slate-900">{{ $arrival->currency }}
                                            {{ number_format(round((float) $totalValue, 2, PHP_ROUND_HALF_UP), 2) }}</div>
                                    </td>
                                    <td class="px-4 py-4">
                                        <div class="flex justify-center gap-2">
                                            ...
                                            <a href="{{ route('departures.edit', $arrival) }}"
                                                class="inline-flex items-center justify-center w-10 h-10 bg-slate-200 hover:bg-slate-300 text-slate-700 rounded-lg transition-colors"
                                                title="Edit" aria-label="Edit departure">
                                                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none"
                                                    viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                                    <path stroke-linecap="round" stroke-linejoin="round"
                                            ...
                                            <a href="{{ route('departures.invoice', $arrival) }}" target="_blank"
                                                class="inline-flex items-center justify-center w-10 h-10 bg-slate-600 hover:bg-slate-700 text-white rounded-lg transition-colors"
                                                title="Print Invoice">
                                                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none"
                                                    viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                                    <path stroke-linecap="round" stroke-linejoin="round"
                                            ...
                                            <form action="{{ route('departures.destroy', $arrival) }}" method="POST"
                                                onsubmit="return confirm('Yakin hapus departure ini?');">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit"
                                                    class="inline-flex items-center justify-center w-10 h-10 bg-red-600 hover:bg-red-700 text-white rounded-lg transition-colors"
                                                    aria-label="Delete departure">
                                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none"
                                                        viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                                        <path stroke-linecap="round" stroke-linejoin="round"
                                                            d="M6 7h12m-9 4v5m6-5v5M9 7l.867-2.6A1 1 0 0 1 10.81 3.5h2.38a1 1 0 0 1 .943.9L15 7m-9 0h12v12a1 1 0 0 1-1 1H8a1 1 0 0 1-1-1V7Z" />
                                                    </svg>
                                                </button>
                                            </form>
                                        </div>
                            ...
                            @empty
                                <tr>
                                    <td colspan="8" class="px-4 py-12 text-center">
                                        ...
                                        <a href="{{ route('departures.create') }}"
                                            class="text-blue-600 hover:text-blue-700 text-sm font-medium">Create your first
                                            departure →</a>
                                    </td>
                                </tr>
                            @endforelse
