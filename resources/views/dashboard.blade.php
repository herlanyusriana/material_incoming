    ...
    <div class="py-6">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                <div
                    class="bg-gradient-to-br from-blue-50 to-white border border-slate-200 rounded-2xl shadow-lg shadow-blue-500/10 p-5 hover:shadow-xl transition-shadow">
                    <div class="flex items-center justify-between">
                        <div>
                            <div class="text-xs uppercase tracking-wider text-slate-600 font-semibold">Total Departures
                            </div>
                            <div class="mt-2 text-3xl font-bold text-blue-600">
                                {{ number_format($summary['total_departures']) }}</div>
                ...
                <div
                    class="bg-gradient-to-br from-green-50 to-white border border-slate-200 rounded-2xl shadow-lg shadow-green-500/10 p-5 hover:shadow-xl transition-shadow">
                    <div class="flex items-center justify-between">
                        <div>
                            <div class="text-xs uppercase tracking-wider text-slate-600 font-semibold">Total Receives
                            </div>
                            <div class="mt-2 text-3xl font-bold text-green-600">
                                {{ number_format($summary['total_receives']) }}</div>
                ...
                <div
                    class="bg-gradient-to-br from-orange-50 to-white border border-slate-200 rounded-2xl shadow-lg shadow-orange-500/10 p-5 hover:shadow-xl transition-shadow">
                    <div class="flex items-center justify-between">
                        <div>
                            <div class="text-xs uppercase tracking-wider text-slate-600 font-semibold">Pending Items
                            </div>
                            <div class="mt-2 text-3xl font-bold text-orange-600">
                                {{ number_format($summary['pending_items']) }}</div>
                            <div class="text-xs text-slate-500 mt-1">Need processing</div>
                        </div>
                        <div
                            class="w-12 h-12 bg-gradient-to-br from-orange-500 to-orange-600 rounded-xl flex items-center justify-center shadow-lg shadow-orange-500/30">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 text-white" fill="none"
                                viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                <path stroke-linecap="round" stroke-linejoin="round"
                ...
                <div
                    class="bg-gradient-to-br from-purple-50 to-white border border-slate-200 rounded-2xl shadow-lg shadow-purple-500/10 p-5 hover:shadow-xl transition-shadow">
                    <div class="flex items-center justify-between">
                        <div>
                            <div class="text-xs uppercase tracking-wider text-slate-600 font-semibold">Today Receives
                            </div>
                            <div class="mt-2 text-3xl font-bold text-purple-600">
                                {{ number_format($summary['today_receives']) }}</div>
                            ...
                            <div
                                ...
                                <div class="flex-1">
                                    ...
                                    <div class="text-xs text-slate-600">
                                        ...
                                        {{ $receive->arrivalItem?->arrival?->vendor?->vendor_name ?? '' }}</div>
                                </div>
            ...
            <div class="bg-white border border-slate-200 rounded-2xl shadow-lg p-6">
                ...
                <div class="mt-5 space-y-4">
                    @forelse ($departures as $arrival)
                        ...
                        @endphp
                        <div
                            class="border border-slate-200 rounded-xl p-5 hover:shadow-md transition-shadow bg-gradient-to-r from-white to-slate-50">
                            <div class="flex items-start justify-between gap-4">
                                <div class="flex-1">
                                    <div class="flex items-center gap-3 mb-2">
                                        ...
                                        <span class="text-xs text-slate-500">by
                                            {{ $arrival->creator->name ?? 'System' }}</span>
                                    </div>
                                    <div class="grid md:grid-cols-2 gap-2 text-sm">
                                        <div>
                                            <span class="text-slate-600">Vendor:</span>
                                            <span
                                                class="font-semibold text-slate-900">{{ $arrival->vendor->vendor_name }}</span>
                                        </div>
                                        <div>
                                            <span class="text-slate-600">Items:</span>
                                            <span class="font-medium text-slate-900">{{ $totalItems }}
                                                item{{ $totalItems != 1 ? 's' : '' }}</span>
                                            <span class="text-slate-500">({{ number_format($totalQty) }} pcs total)</span>
                                        </div>
                                        <div>
                                            <span class="text-slate-600">Total Value:</span>
                                            <span class="font-bold text-blue-600">{{ $arrival->currency }}
                                    ...
                                    <div class="mt-3">
                                        <div class="flex items-center justify-between text-xs mb-1">
                                            ...
                                            <span class="font-bold text-blue-600">{{ $totalReceived }} /
                                                {{ number_format($totalQty) }}</span>
                                        </div>
                                        <div class="w-full bg-slate-200 rounded-full h-2.5 overflow-hidden">
                                            <div class="h-full rounded-full transition-all duration-500 {{ $progress == 100 ? 'bg-green-500' : 'bg-blue-500' }}"
                                ...
                                <div class="flex flex-col gap-2">
                                    <span
                                        class="text-xs text-slate-500">{{ $arrival->invoice_date->format('d M Y') }}</span>
                                    <a href="{{ route('departures.show', $arrival) }}"
                                        class="px-4 py-2 bg-blue-500 hover:bg-blue-600 text-white text-sm font-medium rounded-lg transition-colors text-center">
                                        View Details
                                    </a>
                                </div>
                            </div>
