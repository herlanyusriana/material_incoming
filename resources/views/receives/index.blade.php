                    ...
                    <table class="min-w-full divide-y divide-slate-200">
                        <thead class="bg-gradient-to-r from-slate-50 to-slate-100">
                            <tr>
                                ...
                                <th class="px-4 py-3 text-left text-xs font-semibold text-slate-600 uppercase tracking-wider">ATA</th>
                            ...
                            @forelse ($pendingItems as $item)
                                <tr class="hover:bg-slate-50 transition-colors">
                                    <td class="px-4 py-4 text-sm font-semibold text-slate-900">
                                        {{ $item->arrival->vendor->vendor_name }}
                                    </td>
                                    <td class="px-4 py-4 text-sm text-slate-700">
                                        {{ $item->arrival->invoice_no }}
                                        <div class="text-xs text-slate-500">{{ $item->arrival->arrival_no }}</div>
                                    </td>
                                    <td class="px-4 py-4 text-sm">
                                        <div class="font-semibold text-slate-900">{{ $item->part->part_no }}</div>
                                        <div class="text-xs text-slate-600">{{ $item->part->part_name_vendor }}</div>
                                    </td>
                                    <td class="px-4 py-4 text-sm text-slate-700 font-mono">
                                        {{ $item->size ?? '-' }}
                                    </td>
                                    <td class="px-4 py-4 text-sm text-slate-700">
                                        {{ $item->arrival->ETD ? $item->arrival->ETD->format('d M Y') : '-' }}
                                    </td>
                                    <td class="px-4 py-4 text-sm text-slate-700">
                                        @php
                                            $latestReceive = $item->receives->sortByDesc('ata_date')->first();
                                        @endphp
                                        {{ $latestReceive?->ata_date?->format('d M Y H:i') ?? '-' }}
                                    </td>
                                    <td class="px-4 py-4 text-center">
                                        <a href="{{ route('receives.create', $item) }}" class="inline-flex items-center px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white text-sm font-semibold rounded-lg transition-colors shadow-sm">
