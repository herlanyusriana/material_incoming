...
class WODetailScreenState extends State<WODetailScreen> {
  ...
  Future<void> _scanTag() async {
    ...
    try {
      ...
      if (!mounted) return;
      messenger.showSnackBar(const SnackBar(content: Text('Tag tidak ditemukan')));
    } finally {
  ...
  Future<void> _allocate() async {
    ...
    if (reqId == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Tag tidak terhubung ke requirement WO ini')),
      );
      return;
    }
  ...
  Future<void> _postResult() async {
    ...
    if (good <= 0) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Isi qty Good minimal 1 untuk posting hasil')),
      );
      return;
    }
    setState(() => _busy = true);
    ...
    if (ok) {
      ...
      messenger.showSnackBar(const SnackBar(content: Text('Gagal posting hasil')));
    }
  }

  Future<void> _closeWO() async {
    if (_woId == null || _busy) return;
    final okConfirm = await _confirm('Tutup WO?', 'WO yang sudah ditutup tidak bisa diproses lagi.');
    ...
    if (!mounted) return;
    setState(() => _busy = false);
    messenger.showSnackBar(SnackBar(content: Text(ok ? 'WO ditutup' : 'Gagal menutup WO')));
  }
  ...
  Future<void> _releaseWO() async {
    ...
    if (!mounted) return;
    setState(() => _busy = false);
    messenger.showSnackBar(SnackBar(content: Text(ok ? 'WO direlease' : 'Gagal release WO')));
  }
  ...
  Future<void> _returnAllocation(Map<String, dynamic> a) async {
    ...
    if (!mounted) return;
    setState(() => _busy = false);
    messenger.showSnackBar(SnackBar(content: Text(ok ? 'Alokasi dikembalikan' : 'Gagal return alokasi')));
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: Text(wo != null ? wo['work_order_no'] ?? 'WO Detail' : 'Detail')),
              ...
              : SingleChildScrollView(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      const Divider(),
                      const Text('Scan & Alokasi', style: TextStyle(fontWeight: FontWeight.bold)),
                      Row(
                        children: [
                          Expanded(
                            child: TextField(
                              controller: _tagCtrl,
                              decoration: const InputDecoration(labelText: 'Tag', hintText: 'Scan QR'),
                            ),
                          ),
                          IconButton(
                            tooltip: 'Cari tag',
                      ...
                      if (_tagInfo != null) ...[
                        Text('Tag: ${_tagInfo!['tag']} — ${_tagInfo!['part_no']}'),
                        Text('Stok: ${_tagInfo!['locations']?.first['qty_on_hand'] ?? 0}'),
                        TextField(
                          controller: _qtyCtrl,
                      ...
                      const SizedBox(height: 16),
                      const Divider(),
                      const Text('Posting Hasil', style: TextStyle(fontWeight: FontWeight.bold)),
                      Row(
                        children: [
                          Expanded(
                            child: TextField(
                      ...
                      const SizedBox(height: 16),
                      const Divider(),
                      const Text('Alokasi', style: TextStyle(fontWeight: FontWeight.bold)),
                      ...?wo['allocations']?.map<Widget>((a) => ListTile(
                            title: Text(a['tag'] ?? '-'),
                            subtitle: Text('${a['qty_reserved']} — ${a['status']}'),
                            trailing: a['status'] == 'RESERVED'
                                ? IconButton(
                                    tooltip: 'Kembalikan stok alokasi',
                                    icon: const Icon(Icons.undo, color: Colors.orange),
                                    onPressed: _busy ? null : () => _returnAllocation(a),
                                  )
                                : null,
                          ))?.toList(),
                      const SizedBox(height: 16),
