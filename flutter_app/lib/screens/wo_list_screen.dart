...
class WOListScreenState extends State<WOListScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(_filterStatus.isEmpty ? 'WO Tracking' : 'WO Tracking · $_filterStatus'),
        actions: [
          ...
          IconButton(
            ...
            onPressed: woProv.isLoading
                ? null
                : () => woProv.loadWorkOrders(status: _filterStatus.isNotEmpty ? _filterStatus : null),
          ),
          IconButton(
            tooltip: 'Keluar',
      ...
      body: woProv.isLoading && woProv.workOrders.isEmpty
          ...
          : woProv.error != null
              ? _ErrorView(
                  ...
                  onRetry: () => woProv.loadWorkOrders(status: _filterStatus.isNotEmpty ? _filterStatus : null),
                )
              : RefreshIndicator(
                  onRefresh: () => woProv.loadWorkOrders(status: _filterStatus.isNotEmpty ? _filterStatus : null),
                  child: woProv.workOrders.isEmpty
                      ...
                      : ListView.builder(
                          ...
                          itemBuilder: (ctx, i) {
                            ...
                            return ListTile(
                              title: Text(wo['work_order_no'] ?? '-'),
                              subtitle: Text('${wo['part_no'] ?? '-'} — ${wo['status'] ?? '-'}'),
                              trailing: Text('${wo['qty_target'] ?? 0} ${wo['uom'] ?? ''}'),
                              onTap: () {
                                Navigator.pushNamed(
                                  context,
                                  '/wo-detail',
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 12),
          Text(filterStatus.isEmpty ? 'Belum ada WO' : 'Tidak ada WO berstatus $filterStatus'),
