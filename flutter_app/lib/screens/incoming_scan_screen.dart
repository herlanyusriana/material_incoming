...
class IncomingScanScreenState extends State<IncomingScanScreen> {
  ...
  Future<void> _loadDepartures() async {
    ...
    try {
      ...
      messenger.showSnackBar(const SnackBar(content: Text('Gagal memuat daftar departure')));
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    final selectedArrival = arrivals.where((a) => a['id'] == _selectedArrivalId).firstOrNull;
    final items = selectedArrival?['items']?.cast<Map<String, dynamic>>() ?? const [];
              ...
              child: ListView(
                ...
                children: [
                  DropdownButtonFormField<Map<String, dynamic>>(
                    decoration: const InputDecoration(labelText: 'Pilih Invoice (Departure)'),
                    items: arrivals.map((a) {
                      return DropdownMenuItem<Map<String, dynamic>>(
                        value: a,
                        child: Text('${a['invoice_no'] ?? a['arrival_no'] ?? a['id']} — ${a['vendor_name'] ?? '-'}'),
                      );
                    }).toList(),
                  ...
                  TextField(
                    controller: _qtyCtrl,
                    decoration: const InputDecoration(labelText: 'Qty', helperText: 'Default sisa qty item'),
