...
class ApiService {
  ...
  Future<void> scanReceive(
      int arrivalItemId, String tag, double qty, String qcStatus) async {
    await _dio
        .post('/api/incoming/arrival-items/$arrivalItemId/receive', data: {
  ...
  Future<Map<String, dynamic>> locateTag(int woId, String tag) async {
    final response =
        await _dio.post('/api/wo-tracking/$woId/locate-tag', data: {
      'tag': tag,
    });
  ...
  // Allocate
  Future<void> allocate(int woId, int requirementId, String tag, double qty,
