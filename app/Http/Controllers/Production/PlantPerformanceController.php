...
class PlantPerformanceController extends Controller
{
    public function index(Request $request)
    {
        ...
        $departments = [
            'Production' => [
                ['name' => 'Production Achievement (%)', 'value' => $productionAchievement, 'suffix' => '%', 'formula' => '(Actual Production / Planned Production) x 100%'],
                ['name' => 'Efficiency (%)', 'value' => $efficiency, 'suffix' => '%', 'formula' => '(STD Time / Act Time) x 100%'],
                ['name' => 'Machine Load (hours)', 'value' => $machineLoadHours, 'suffix' => 'h', 'formula' => 'Act. time machine used'],
                ['name' => 'Machine Capacity (hours)', 'value' => $machineCapacityHours, 'suffix' => 'h', 'formula' => 'Available machine hours on used machine/day'],
                ['name' => 'Utilization Machine (%)', 'value' => $utilization, 'suffix' => '%', 'formula' => '(Machine Load / Machine Capacity) x 100%'],
                ['name' => 'Overall Equipment Effectiveness (%)', 'value' => $oee, 'suffix' => '%', 'formula' => 'Availability x Performance x Quality'],
                ['name' => 'Loss Time (Minute)', 'value' => $lossTimeMinutes, 'suffix' => 'min', 'formula' => 'Planned production time - operating time'],
            ],
            'Material' => [
                ['name' => 'Inventory Accuracy (%)', 'value' => $inventoryAccuracy, 'suffix' => '%', 'formula' => 'Different count / Total count x 100%'],
                ['name' => 'Shortage RM (Minute)', 'value' => $shortageRmMinutes, 'suffix' => 'min', 'formula' => 'Late delivery to production'],
            ],
            'Logistics' => [
                ['name' => 'Delivery Shortage (Minute)', 'value' => $deliveryShortageMinutes, 'suffix' => 'min', 'formula' => 'Late delivery to customers'],
                ['name' => 'Quality Defect in Transport (IDR)', 'value' => $transportDefectCost, 'suffix' => 'IDR', 'formula' => 'No good due to handling'],
            ],
            'Quality' => [
                ['name' => 'Defect Rate (%)', 'value' => $this->safePercent($ngQty, $actualQty), 'suffix' => '%', 'formula' => 'Total no good conditions / Total production output x 100%'],
                ['name' => 'Failure Cost (IDR)', 'value' => $failureCost, 'suffix' => 'IDR', 'formula' => 'Defect quantity x sales price'],
            ],
        ];
        ...
        $summary = [
            ...
            'transport_defect_source_ready' => false,
        ];

        return view('production.plant-performance.index', compact(
            'dateFrom',
            'dateTo',
            'departments',
            'summary',
