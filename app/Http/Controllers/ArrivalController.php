...
class ArrivalController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $validated = $request->validate([
            ...
            'items.*.unit_bundle' => ['nullable', 'string', 'max:20', Rule::in(['BUNDLE', 'PALLET', 'BOX', 'BAG', 'ROLL', 'PACKAGE'])],
        ...
        $arrival = DB::transaction(function () use ($validated, $vendorId, $billOfLadingFilePath) {
            ...
            $arrivalData = [
                ...
                'pen_no' => isset($validated['pen_no']) ? strtoupper(trim((string) $validated['pen_no'])) : null,
                'pen_date' => $validated['pen_date'] ?? null,
                'aju_no' => isset($validated['aju_no']) ? strtoupper(trim((string) $validated['aju_no'])) : null,
    ...
    public function storeItem(Request $request, Arrival $departure)
    {
        ...
        $data = $request->validate([
            ...
            'unit_bundle' => ['nullable', 'string', 'max:20', Rule::in(['BUNDLE', 'PALLET', 'BOX', 'BAG', 'ROLL', 'PACKAGE'])],
